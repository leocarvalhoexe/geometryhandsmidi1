// ==========================================================================
// MIDI SHAPE MANIPULATOR v37 - main37.js
// ==========================================================================

// === GLOBAL VARIABLES & CONSTANTS ===
const sidebar = document.getElementById('sidebar');
const sidebarHandle = document.getElementById('sidebarHandle');
const mainCanvasContainer = document.getElementById('mainCanvasContainer'); // For touch outside to close
...
function setupEventListeners() {
    ...
    if (sidebar && sidebarHandle) {
        ...
        if (!isTouchDevice) { // Desktop hover behavior
            ...
            // Keep handle outside, so it's always hoverable to open
            // The #sidebarHandle itself is part of the #sidebar visually but positioned to allow this
        } else { // Mobile touch behavior
             // Ensure handle is styled for mobile visibility if it's different
            ...
            document.addEventListener('click', (event) => {
             ...
             // Also handle touch events for closing when touching outside
            document.addEventListener('touchstart', (event) => {
                if (sidebar.classList.contains('open') && !sidebar.contains(event.target) && event.target !== sidebarHandle) {
                    // Check if the touch is not on any button inside the sidebar either
                    let targetElement = event.target;
                    let isInsideSidebarButton = false;
                    while (targetElement && targetElement !== document.body) {
                        if (targetElement.classList && targetElement.classList.contains('control-button') && sidebar.contains(targetElement)) {
                            isInsideSidebarButton = true;
                            break;
                        }
                        targetElement = targetElement.parentNode;
                    }
                    if(!isInsideSidebarButton) {
                        sidebar.classList.remove('open');
                    }
                }
    ...
    const infoButton = document.getElementById('info'); // This button is now in the sidebar
    const closeModalButton = document.getElementById('closeModal'); // Close button for the info modal
    const infoModal = document.getElementById('infoModal');
...
window.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Carregado. Iniciando main37.js..."); // Updated version in log
    ...
    console.log("main37.js inicialização completa."); // Updated version in log
});
// === END INITIALIZATION ===

// === MANUAL UPDATE ===
function updateManualText() {
    const infoModalContent = document.querySelector('#infoModal .modal-content');
    if (infoModalContent) {
        // Find the h3 to change version
        const titleElement = infoModalContent.querySelector('h3');
        if (titleElement) {
            titleElement.textContent = 'Manual do Usuário - v37';
        }

        // Create new section for v37
        const v37Changes = `
            <h4>Novidades da v37: Interface com Painel Lateral Retrátil</h4>
            <ul>
        ...
        // Find the first h4 to insert before it
        const firstH4 = infoModalContent.querySelector('h4');
        if (firstH4) {
            firstH4.insertAdjacentHTML('beforebegin', v37Changes);
        } else {
            // If no h4, append to the content (less ideal)
            const closeButton = infoModalContent.querySelector('.close-modal-button');
            if (closeButton) {
                 closeButton.insertAdjacentHTML('afterend', v37Changes); // Insert after close button, before other content if any
            } else {
                infoModalContent.innerHTML += v37Changes; // Fallback
            }
        }
    }
}

// Call this function during initialization, for example, at the end of DOMContentLoaded
window.addEventListener('DOMContentLoaded', () => {
