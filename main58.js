// ==========================================================================
// MIDI SHAPE MANIPULATOR v58 - main58.js
// ==========================================================================

// simpleSynth e audioCtx são gerenciados e obtidos de synth58.js (ou versão compatível)
...
function processShapeNotes(shape, isPulsing, pulseValue) {
    ...
    if (canPlayNonArp || canPlayArp) {
        ...
        // V58: Desligar todas as notas ativas anteriores da forma, exceto em modo staccato
        if (!staccatoModeActive) {
            Object.keys(shape.activeMidiNotes).forEach(key => {
                ...
                if (noteInfo && noteInfo.playing) {
                    ...
                    // Não deletar a nota aqui ainda, a lógica de limpeza no final de drawShape cuidará disso
                    // ou a própria nota será substituída se for a mesma a ser tocada.
                }
            });
        }
        // Fim da modificação V58

        if (currentNoteMode !== 'CHORD' && currentNoteMode !== 'ARPEGGIO') {
            const oldKey = `${edgeIdx}_0`;
            // A lógica de desligar notas foi movida para cima (V58), então esta verificação específica pode não ser mais estritamente necessária
            // se a intenção era apenas desligar a nota no oldKey. Contudo, mantendo para consistência caso haja
            // alguma nuance onde a nota no oldKey precise ser explicitamente desligada antes do switch.
            // No entanto, com a nova lógica, a nota em oldKey já teria sido desligada.
            // Para evitar duplo noteOff ou comportamento inesperado, esta linha pode ser comentada ou removida se
            // a lógica V58 cobrir todos os casos. Por segurança, vamos manter, mas cientes da possível redundância.
            if (shape.activeMidiNotes[oldKey]?.playing && !staccatoModeActive) {
                 // sendMidiNoteOff(shape.activeMidiNotes[oldKey].note, shape.midiChannel, shape.id + 1); // Já tratado acima
                 // shape.activeMidiNotes[oldKey].playing = false; // Já tratado acima
            }
        }
        switch (currentNoteMode) {
            case 'SEQUENTIAL':
                if (canPlayNonArp && shape.sides > 0) {
                    // A lógica de desligar notas anteriores foi movida para o bloco V58 acima.
                    // Não é mais necessário o loop específico aqui para desligar notas sequenciais.
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main55.js...");
  console.log("DOM Carregado. Iniciando main58.js (v58)..."); // Log de versão atualizado
    detectPlatform();
  ...
  // podemos tentar aplicar as configurações de áudio imediatamente.
    ...
    console.log("AudioContext já estava rodando ou foi ativado rapidamente. Aplicando configurações do synth (v58).");
  ...
  if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas (v58).\n`;

  console.log("Iniciando loop de animação (v58) e finalizando DOMContentLoaded.");
    animationLoop();
});
