// ==========================================================================
// MIDI SHAPE MANIPULATOR v50 - main50.js
// Integração dos novos recursos do synth50.js
// AGORA UTILIZANDO MÓDULOS ES6
// ==========================================================================

import {
    SimpleSynth, // Embora não instanciemos diretamente, pode ser útil para type hinting ou futuras refatorações
    initAudioContext, // Pode não ser chamado diretamente se getAudioContext/getSimpleSynthInstance cuidarem disso
    initAudioContextOnGesture,
    setInternalAudioEnabledState,
    getSimpleSynthInstance,
    getAudioContext,
    midiToFrequency // Se usado diretamente em main50.js, senão pode ser removido da importação
} from './synth50.js';

// simpleSynth e audioCtx são agora obtidos EXCLUSIVAMENTE através das funções importadas.
...
// === V50: Synth Control Sidebar Elements (Delay & Reverb) ===
let scDelayEnableCheckbox = document.getElementById('scDelayEnable');
...
let scReverbEnableCheckbox = document.getElementById('scReverbEnable');
let scReverbIRSelect = document.getElementById('scReverbIR');
let scReverbMixSlider = document.getElementById('scReverbMix');
let scReverbMixValue = document.getElementById('scReverbMixValue');

// === V50: Synth Control Sidebar Elements (LFO) ===
let scLfoWaveformSelect = document.getElementById('scLfoWaveform');
let scLfoFrequencySlider = document.getElementById('scLfoFrequency');
let scLfoFrequencyValue = document.getElementById('scLfoFrequencyValue');
let scLfoDepthSlider = document.getElementById('scLfoDepth');
let scLfoDepthValue = document.getElementById('scLfoDepthValue');
let scLfoTargetSelect = document.getElementById('scLfoTarget');
// === END V50 Elements ===
...
function setupEventListeners() {
    ...
    if (settingsButton && settingsModal) settingsButton.addEventListener('click', () => { settingsModal.style.display = 'flex'; updateModalSynthControls(); }); // V48: Update modal on open
...
function handleSynthControlChange(param, value) {
    ...
    switch (param) {
        ...
        // --- V50: Delay Controls ---
        case 'delayEnable':
            synth.setDelayEnabled(value);
            if (scDelayEnableCheckbox) scDelayEnableCheckbox.checked = value;
            // TODO: Add to modal if exists
            break;
        case 'delayTime':
        ...
        // --- V50: Reverb Controls ---
        case 'reverbEnable':
            synth.setReverbEnabled(value);
            if (scReverbEnableCheckbox) scReverbEnableCheckbox.checked = value;
            // TODO: Add to modal if exists
            break;
        case 'reverbIR':
            if (value) { // value is URL of IR
                synth.loadReverbImpulseResponse(value);
            } else { // "None" selected
                synth.setReverbEnabled(false); // Disable reverb if no IR
                if(scReverbEnableCheckbox) scReverbEnableCheckbox.checked = false;
            }
            if (scReverbIRSelect) scReverbIRSelect.value = value;
            break;
        case 'reverbMix':
        ...
        case 'lfoFrequency':
            ...
            if (scLfoFrequencySlider) scLfoFrequencySlider.value = value;
            if (scLfoFrequencyValue) scLfoFrequencyValue.textContent = `${value.toFixed(1)}Hz`;
            break;
        case 'lfoDepth':
            synth.setLfoDepth(value);
            if (scLfoDepthSlider) scLfoDepthSlider.value = value;
            if (scLfoDepthValue) scLfoDepthValue.textContent = value.toFixed(2);
            break;
        case 'lfoTarget':
            synth.setLfoTarget(value);
            if (scLfoTargetSelect) scLfoTargetSelect.value = value;
            break;
    }
...
function updateSidebarSynthControls() {
    const synth = getSimpleSynthInstance();
    if (!synth || !synthControlsSidebar) return; // Add !synthControlsSidebar check
    ...
    // V50: Update Sidebar Delay Controls
    if (scDelayEnableCheckbox) scDelayEnableCheckbox.checked = synth.delayEnabled;
    ...
    if (scDelayMixSlider) scDelayMixSlider.value = synth.delayWetGain.gain.value; // Assuming wet gain represents overall mix
    if (scDelayMixValue) scDelayMixValue.textContent = synth.delayWetGain.gain.value.toFixed(2);

    // V50: Update Sidebar Reverb Controls
    if (scReverbEnableCheckbox) scReverbEnableCheckbox.checked = synth.reverbEnabled;
    if (scReverbIRSelect && synth.loadedIRUrl) scReverbIRSelect.value = synth.loadedIRUrl; // Need to store loaded IR URL in synth
    else if (scReverbIRSelect) scReverbIRSelect.value = ""; // Default to "None"
    if (scReverbMixSlider) scReverbMixSlider.value = synth.reverbWetGain.gain.value;
    if (scReverbMixValue) scReverbMixValue.textContent = synth.reverbWetGain.gain.value.toFixed(2);

    // V50: Update Sidebar LFO Controls
    if (scLfoWaveformSelect) scLfoWaveformSelect.value = synth.lfoWaveform;
    if (scLfoFrequencySlider) scLfoFrequencySlider.value = synth.lfoFrequency;
    if (scLfoFrequencyValue) scLfoFrequencyValue.textContent = `${synth.lfoFrequency.toFixed(1)}Hz`;
    if (scLfoDepthSlider) scLfoDepthSlider.value = synth.lfoDepth;
    if (scLfoDepthValue) scLfoDepthValue.textContent = synth.lfoDepth.toFixed(2);
    if (scLfoTargetSelect) scLfoTargetSelect.value = synth.lfoTarget;
}
...
function initSynthControlsSidebar() {
    ...
    // V50: Get Delay & Reverb elements
    scDelayEnableCheckbox = document.getElementById('scDelayEnable');
    scDelayTimeSlider = document.getElementById('scDelayTime');
    scDelayTimeValue = document.getElementById('scDelayTimeValue');
    scDelayFeedbackSlider = document.getElementById('scDelayFeedback');
    scDelayFeedbackValue = document.getElementById('scDelayFeedbackValue');
    scDelayMixSlider = document.getElementById('scDelayMix');
    scDelayMixValue = document.getElementById('scDelayMixValue');
    scReverbEnableCheckbox = document.getElementById('scReverbEnable');
    scReverbIRSelect = document.getElementById('scReverbIR');
    scReverbMixSlider = document.getElementById('scReverbMix');
    scReverbMixValue = document.getElementById('scReverbMixValue');

    // V50: Get LFO elements
    scLfoWaveformSelect = document.getElementById('scLfoWaveform');
    scLfoFrequencySlider = document.getElementById('scLfoFrequency');
    scLfoFrequencyValue = document.getElementById('scLfoFrequencyValue');
    scLfoDepthSlider = document.getElementById('scLfoDepth');
    scLfoDepthValue = document.getElementById('scLfoDepthValue');
    scLfoTargetSelect = document.getElementById('scLfoTarget');
    ...
    // V50: Add listeners for Delay & Reverb controls
    if (scDelayEnableCheckbox) scDelayEnableCheckbox.addEventListener('change', (e) => handleSynthControlChange('delayEnable', e.target.checked));
    ...
    if (scReverbEnableCheckbox) scReverbEnableCheckbox.addEventListener('change', (e) => handleSynthControlChange('reverbEnable', e.target.checked));
    if (scReverbIRSelect) scReverbIRSelect.addEventListener('change', (e) => handleSynthControlChange('reverbIR', e.target.value));
    ...
    // V50: Add listeners for LFO controls
    if (scLfoWaveformSelect) scLfoWaveformSelect.addEventListener('change', (e) => handleSynthControlChange('lfoWaveform', e.target.value));
    if (scLfoFrequencySlider) scLfoFrequencySlider.addEventListener('input', (e) => handleSynthControlChange('lfoFrequency', parseFloat(e.target.value)));
    if (scLfoDepthSlider) scLfoDepthSlider.addEventListener('input', (e) => handleSynthControlChange('lfoDepth', parseFloat(e.target.value)));
    if (scLfoTargetSelect) scLfoTargetSelect.addEventListener('change', (e) => handleSynthControlChange('lfoTarget', e.target.value));

    // V49: Logic for toggleSynthPanelButton (button is now created in HTML)
    if (toggleSynthPanelButton && synthControlsSidebar) {
        const isSynthPanelHidden = loadPersistentSetting('synthPanelHidden', false);
        if (isSynthPanelHidden) {
            synthControlsSidebar.style.display = 'none';
        }
        ...
        toggleSynthPanelButton.addEventListener('click', () => {
            ...
            synthControlsSidebar.style.display = isHidden ? 'block' : 'none'; // Use 'block' or 'flex' based on original display style
            toggleSynthPanelButton.classList.toggle('active', isHidden);
            savePersistentSetting('synthPanelHidden', !isHidden);
            logOSC("SYSTEM", "Painel Synth Alternado", [isHidden ? "Mostrando" : "Ocultando"]);
        });
    } else {
        console.warn("toggleSynthPanelButton ou synthControlsSidebar não encontrado no initSynthControlsSidebar.");
    }

    updateSidebarSynthControls(); // Initial population
    console.log("Synth Control Sidebar initialized."); // V49: Message updated
}


let internalAudioEnabled = true; // Default to true, will be loaded from settings
...
function toggleInternalAudio() {
  ...
  const currentSimpleSynth = getSimpleSynthInstance();
  if (typeof setInternalAudioEnabledState === "function") { // from synth48.js
...
function handleKeyPress(e) {
  ...
  const correctedShiftActionMap = {
    ...
    'C': () => { if (settingsModal) { settingsModal.style.display = settingsModal.style.display === 'flex' ? 'none' : 'flex'; if(settingsModal.style.display === 'flex') updateModalSynthControls(); } }, // V48
    ...
    'T': toggleTheme,
    'Y': () => { if (toggleSynthPanelButton) toggleSynthPanelButton.click(); }, // V49: Atalho para painel do synth
  };
...
function saveAllPersistentSettings(){
  ...
  if(currentSimpleSynth) {
    ...
    // V50: Save Delay settings
    savePersistentSetting('delayEnabled', currentSimpleSynth.delayEnabled);
    ...
    savePersistentSetting('delayMix', currentSimpleSynth.delayWetGain.gain.value);
    // V50: Save Reverb settings
    savePersistentSetting('reverbEnabled', currentSimpleSynth.reverbEnabled);
    savePersistentSetting('reverbIR', currentSimpleSynth.loadedIRUrl || ""); // Save current IR URL
    savePersistentSetting('reverbMix', currentSimpleSynth.reverbWetGain.gain.value);
    // V50: Save LFO settings
    savePersistentSetting('lfoWaveform', currentSimpleSynth.lfoWaveform);
    savePersistentSetting('lfoFrequency', currentSimpleSynth.lfoFrequency);
    savePersistentSetting('lfoDepth', currentSimpleSynth.lfoDepth);
    savePersistentSetting('lfoTarget', currentSimpleSynth.lfoTarget);
  }
...
function loadAllPersistentSettings(){
  ...
  // V50: Load Delay settings
  const savedDelayEnabled = loadPersistentSetting('delayEnabled', false);
  const savedDelayTime = loadPersistentSetting('delayTime', 0.5);
  const savedDelayFeedback = loadPersistentSetting('delayFeedback', 0.5);
  const savedDelayMix = loadPersistentSetting('delayMix', 0.4);
  // V50: Load Reverb settings
  const savedReverbEnabled = loadPersistentSetting('reverbEnabled', false);
  const savedReverbIR = loadPersistentSetting('reverbIR', "");
  const savedReverbMix = loadPersistentSetting('reverbMix', 0.3);
  // V50: Load LFO settings
  const savedLfoWaveform = loadPersistentSetting('lfoWaveform', 'sine');
  const savedLfoFrequency = loadPersistentSetting('lfoFrequency', 5);
  const savedLfoDepth = loadPersistentSetting('lfoDepth', 0);
  const savedLfoTarget = loadPersistentSetting('lfoTarget', 'none');
  ...
  // Aplica ao synth APÓS sua inicialização (geralmente via firstGestureHandler)
  // Aqui, apenas carregamos os valores. A aplicação será feita em firstGestureHandler
  // ou se o synth já existir.

  // Atualizar UI do Modal (se visível ou ao abrir) e Sidebar (após sua inicialização)
  // Os valores já foram carregados acima. updateModalSynthControls e updateSidebarSynthControls
  // serão responsáveis por popular os elementos da UI com esses valores carregados.
  ...
  return {
    ...
    // Retornar valores de áudio para aplicação posterior
    audioSettings: {
        ...
        // V50 settings
        delayEnabled: savedDelayEnabled,
        delayTime: savedDelayTime,
        delayFeedback: savedDelayFeedback,
        delayMix: savedDelayMix,
        reverbEnabled: savedReverbEnabled,
        reverbIR: savedReverbIR,
        reverbMix: savedReverbMix,
        // LFO settings
        lfoWaveform: savedLfoWaveform,
        lfoFrequency: savedLfoFrequency,
        lfoDepth: savedLfoDepth,
        lfoTarget: savedLfoTarget
    }
  };
}
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main49.js..."); // V49 update
    console.log("DOM Carregado. Iniciando main49.js..."); // V49 update
    detectPlatform();
    ...
    initPresetManager();
    setupEventListeners(); // Isso precisa acontecer ANTES de initSynthControlsSidebar se o botão estiver no HTML
    initSynthControlsSidebar(); // V49: Initialize the synth panel logic

    const firstGestureHandler = async () => { // Tornada async
        console.log("Primeiro gesto detectado, tentando inicializar o áudio via synth50.js...");
        if (typeof initAudioContextOnGesture === "function") {
            const audioReady = initAudioContextOnGesture(); // Esta função em si não é async, mas o que vem depois pode ser
            if (audioReady) {
                ...
                if (synthAfterGesture) {
                    ...
                    // V50: Apply Delay and Reverb settings
                    ...
                    synthAfterGesture.setDelayEnabled(audioSettings.delayEnabled); // Enable last, after params

                    if (audioSettings.reverbIR) {
                        await synthAfterGesture.loadReverbImpulseResponse(audioSettings.reverbIR); // Adicionado await
                    }
                    synthAfterGesture.setReverbMix(audioSettings.reverbMix);
                    synthAfterGesture.setReverbEnabled(audioSettings.reverbEnabled && !!audioSettings.reverbIR); // Enable last

                    // V50: Apply LFO settings
                    synthAfterGesture.setLfoWaveform(audioSettings.lfoWaveform);
                    synthAfterGesture.setLfoFrequency(audioSettings.lfoFrequency);
                    synthAfterGesture.setLfoDepth(audioSettings.lfoDepth);
                    synthAfterGesture.setLfoTarget(audioSettings.lfoTarget);


                    // Atualizar UI do Modal e da Sidebar
                    updateModalSynthControls(); // This should also be updated for new controls if they are added to modal
                    updateSidebarSynthControls();
                    console.log("Áudio inicializado/resumido por gesto e synth (ADSR, Distortion, Delay, Reverb) configurado com valores carregados.");
                } else {
                ...
                console.warn("initAudioContextOnGesture() de synth49.js não retornou sucesso. O áudio pode não estar pronto."); // V49 update
            }
        } else {
            console.error("initAudioContextOnGesture não está definida globalmente (esperada de synth49.js)."); // V49 update
        }
    };
    ...
    // Se o AudioContext já estiver rodando (ex: recarregamento rápido da página),
    ...
    if (currentSynth && getAudioContext()?.state === 'running') {
        ...
        // V50: Apply Delay and Reverb settings if AudioContext was already running
        ...
        currentSynth.setDelayEnabled(audioSettings.delayEnabled);
        if (audioSettings.reverbIR) {
            currentSynth.loadReverbImpulseResponse(audioSettings.reverbIR).then(() => {
                 currentSynth.setReverbMix(audioSettings.reverbMix);
                 currentSynth.setReverbEnabled(audioSettings.reverbEnabled && !!audioSettings.reverbIR);
                 updateSidebarSynthControls(); // Update again after IR loaded
            });
        } else {
            currentSynth.setReverbMix(audioSettings.reverbMix);
            currentSynth.setReverbEnabled(false); // No IR, so disable
        }

        updateModalSynthControls();
        updateSidebarSynthControls();
        console.log("AudioContext já estava rodando, configurações do synth (ADSR, Distortion, Delay, Reverb) aplicadas.");
    ...
    if(infoHudButton && hudElement && !hudElement.classList.contains('hidden')) { // V49: Ensure HUD button state matches
