// ==========================================================================
// MIDI SHAPE MANIPULATOR v48 - main48.js
// ==========================================================================

// simpleSynth e audioCtx são gerenciados e obtidos de synth48.js
...
const PRESETS_STORAGE_KEY = 'midiShapePresetsV48'; // ATUALIZADO para v48
let shapePresets = {};
const APP_SETTINGS_KEY = 'midiShapeManipulatorV48Settings'; // ATUALIZADO para v48
const ARPEGGIO_SETTINGS_KEY = 'arpeggioSettingsV48'; // ATUALIZADO para v48
const CAMERA_DEVICE_ID_KEY = 'midiShapeCameraDeviceIdV48'; // ATUALIZADO para v48
...
function initSynthControlsSidebar() {
    ...
    // V48: Add a button to toggle visibility of the synth controls sidebar
    const toggleSynthPanelButton = document.createElement('button');
    toggleSynthPanelButton.id = 'toggleSynthPanelButton';
    toggleSynthPanelButton.classList.add('control-button');
    toggleSynthPanelButton.title = 'Mostrar/Ocultar Painel do Sintetizador';
    toggleSynthPanelButton.innerHTML = '🎛️ Synth Panel';

    const sidebarContent = document.getElementById('sidebarContent');
    if (sidebarContent && internalAudioToggleButton) {
        // Insert it after the "Internal Audio" button for logical grouping
        sidebarContent.insertBefore(toggleSynthPanelButton, internalAudioToggleButton.nextSibling);
    } else if (sidebarContent) {
        sidebarContent.appendChild(toggleSynthPanelButton); // Fallback if internalAudioToggleButton not found
    }

    if (toggleSynthPanelButton && synthControlsSidebar) {
        // Load saved state for synth panel visibility
    ...
    console.log("Synth Control Sidebar initialized with toggle button.");
}
...
function saveAllPersistentSettings(){
  ...
  console.log("Configs V48 salvas no localStorage."); // V48 update
}
...
function loadAllPersistentSettings(){
  ...
  console.log("Configs V48 carregadas do localStorage."); // V48 update
  return {
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main48.js..."); // V48 update
    console.log("DOM Carregado. Iniciando main48.js..."); // V48 update
    detectPlatform();
    ...
    initPresetManager();
    setupEventListeners();
    initSynthControlsSidebar(); // V48: Initialize the new sidebar

    const firstGestureHandler = () => {
        console.log("Primeiro gesto detectado, tentando inicializar o áudio via synth48.js...");
        if (typeof initAudioContextOnGesture === "function") {
            ...
            if (audioReady) {
                ...
                console.warn("initAudioContextOnGesture() de synth48.js não retornou sucesso. O áudio pode não estar pronto.");
            }
        } else {
            console.error("initAudioContextOnGesture não está definida globalmente (esperada de synth48.js).");
        }
    };
    ...
    if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas (v48).\n`; // V48 update

    console.log("Iniciando loop de animação (v48) e finalizando DOMContentLoaded."); // V48 update
    animationLoop();
});
