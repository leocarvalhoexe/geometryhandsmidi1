...
let beatMatrix = {
    ...
    updateGridVisuals: function() { /* ... (adaptado da v73, usa this.domElements.gridElement e classe 'pad') ... */
    ...
    togglePadState: function(row, col) { /* ... (mesma lógica da v73) ... */
        ...
        // Opcional: tocar imediatamente se o pad for ativado na coluna/posição atual de alguma barra
        if (this.isPlaying && this.padStates[row][col] && col === this.currentStep) {
            this.playNoteForPad(row, col, 'main');
        }
        if (this.isPlayingSecondary && this.padStates[row][col]) {
            if (this.secondaryBarOrientation === 'horizontal' && col === this.currentPositionSecondary) {
                this.playNoteForPad(row, col, 'secondary');
            } else if (this.secondaryBarOrientation === 'vertical' && row === this.currentPositionSecondary) {
                 this.playNoteForPad(row, col, 'secondary');
            }
        }
        // TODO: Lógica similar para barras extras
        this.saveSettings();
    },
    ...
    updateBPMFaderVisuals: function(barType, bpmValue, barInstance = null) {
        ...
        if (barType === 'main') {
            ...
            prefix = "BPM Prin: ";
            let displayBPMMain = this.useGlobalBPM && this.getGlobalBPMCallback ? this.getGlobalBPMCallback() : this.currentBPM;
            if (isNaN(parseFloat(displayBPMMain))) displayBPMMain = this.MIN_BPM_FADER;
            if (displayElem) displayElem.textContent = prefix + Math.round(displayBPMMain);
        } else if (barType === 'secondary') {
            ...
            prefix = "BPM Sec: ";
            let effectiveSecBPM = this._calculateEffectiveBpmSecondary();
            if (isNaN(parseFloat(effectiveSecBPM))) effectiveSecBPM = this.MIN_BPM_FADER;
            // O display da barra secundária mostra o BPM *efetivo* (sincronizado)
            // O fader mostra o BPM *base* (this.bpmSecondary)
             if (displayElem) displayElem.textContent = prefix + Math.round(effectiveSecBPM);

            let baseSecondaryBPM = parseFloat(this.bpmSecondary);
            if (isNaN(baseSecondaryBPM)) baseSecondaryBPM = this.MIN_BPM_FADER;
            targetBPM = Math.max(this.MIN_BPM_FADER, Math.min(this.MAX_BPM_FADER, baseSecondaryBPM)); // Fader reflete o valor base
            ...
            let effectiveExtraBPM = this._calculateEffectiveBpmExtra(barInstance);
            if (isNaN(parseFloat(effectiveExtraBPM))) effectiveExtraBPM = this.MIN_BPM_FADER;
            if (displayElem) displayElem.textContent = prefix + Math.round(effectiveExtraBPM);

            let baseExtraBPM = parseFloat(barInstance.bpm);
            if (isNaN(baseExtraBPM)) baseExtraBPM = this.MIN_BPM_FADER;
            targetBPM = Math.max(this.MIN_BPM_FADER, Math.min(this.MAX_BPM_FADER, baseExtraBPM));
        } else {
            return;
        }

        // Ensure targetBPM for fader is absolutely not NaN before proceeding
        if (isNaN(targetBPM)) {
            targetBPM = this.MIN_BPM_FADER;
        }

        if (faderThumb && bpmTextDisplayElem) {
            let normalizedBpm = 0;
            if (this.MAX_BPM_FADER !== this.MIN_BPM_FADER) {
                normalizedBpm = (targetBPM - this.MIN_BPM_FADER) / (this.MAX_BPM_FADER - this.MIN_BPM_FADER);
            }
            // Ensure normalizedBpm is a valid number (0 to 1)
            normalizedBpm = Math.max(0, Math.min(1, isNaN(normalizedBpm) ? 0 : normalizedBpm));
            ...
            // Ensure thumbX is a valid number
            thumbX = Math.max(this.H_BPM_FADER_TRACK_X, Math.min(thumbX, this.H_BPM_FADER_TRACK_X + availableTrackWidth));
            if (isNaN(thumbX)) { // Final fallback for thumbX
                thumbX = this.H_BPM_FADER_TRACK_X;
            }
    ...
    recreateExtraBarControlsDOM: function() {
        ...
        loadedBarsData.forEach(barData => {
            ...
            // Para um restore perfeito, a função addExtraBar precisaria aceitar um objeto de configuração.
    ...
    playNoteForPad: function(row, col, barSource = 'main', noteOffsetOverride = null) {
        ...
        const midiChannel = 9; // Canal 10 para a Beat Matrix
