...
const PRESETS_STORAGE_KEY = 'midiShapePresetsV35';
let shapePresets = {};
const APP_SETTINGS_KEY = 'midiShapeManipulatorV35Settings';
const ARPEGGIO_SETTINGS_KEY = 'arpeggioSettingsV35';
const CAMERA_DEVICE_ID_KEY = 'midiShapeCameraDeviceIdV36';

// DOM Elements
const hudElement = document.getElementById('hud'); // Existing HUD element
const infoHudButton = document.getElementById('infoHudButton'); // New button for HUD visibility
...
let hands; // Declarado globalmente
let camera; // Declarado globalmente
...
async function initializeCamera(deviceId = null) {
    ...
    // Parar a câmera anterior se existir
    if (camera && typeof camera.stop === 'function') {
        try { // Envolver em try-catch caso stop() falhe ou não seja uma promessa
          await camera.stop(); 
        } catch(e) {
          console.warn("Erro ao parar câmera anterior:", e);
        }
        camera = null; // Limpa a referência antiga
    }
...
function onResults(results) {
  logDebug("onResults chamado.", { numHands: results.multiHandLandmarks?.length }); // Log inicial em onResults
  ...
  if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
    logDebug(`Mãos detectadas: ${results.multiHandLandmarks.length}`, results.multiHandedness);
    if (operationMode === 'one_person') {
      ...
      logDebug(`Forma 0 (one_person): LH ${lH ? 'Detectada' : 'Não'}, RH ${rH ? 'Detectada' : 'Não'}`);
      if (shapes.length > 1) { 
      ...
      results.multiHandLandmarks.forEach((landmarks, i) => {
        ...
        for(let j=0; j<shapes.length; j++){
          if(handedness === "Left" && !shapes[j].leftHandLandmarks && !assignedL[j]) {
            ...
            assignedL[j]=true;
            logDebug(`Forma ${j} (two_persons): LH Atribuída`);
            break;
          }
          if(handedness === "Right" && !shapes[j].rightHandLandmarks && !assignedR[j]) {
            shapes[j].rightHandLandmarks = landmarks;
            assignedR[j]=true;
            logDebug(`Forma ${j} (two_persons): RH Atribuída`);
            break;
          }
        }
      });
      shapes.forEach((s, idx) => {
        if (!assignedL[idx] && !assignedR[idx] && !(results.multiHandLandmarks.some((_, k) => (results.multiHandedness[k]?.label === "Left" && shapes[idx].leftHandLandmarks) || (results.multiHandedness[k]?.label === "Right" && shapes[idx].rightHandLandmarks )))) {
            // Apenas loga se NENHUMA mão foi atribuída a esta forma nesta iteração E ela não já tinha uma de antes (cenário raro, mais para consistência)
            logDebug(`Forma ${idx} (two_persons): Nenhuma mão nova atribuída.`);
        }
      });
    }
  } else {
    logDebug("Nenhuma mão detectada em onResults.");
  }

  shapes.forEach(shape => {
    if (spectatorModeActive) {
      shape.activeGesture = null;
      // logDebug(`Forma ${shape.id}: Modo Espectador ATIVO, pulando gestos.`); // Log já coberto pelo HUD
      return;
    }
    ...
    if (wristCount > 0) { // Mover centro da forma com base na(s) mão(s)
      ...
      // logDebug(`Forma ${shape.id}: Movendo centro para X:${targetCenterX.toFixed(1)}, Y:${targetCenterY.toFixed(1)}`); // Pode ser muito verboso
    ...
    if (shape.leftHandLandmarks && shape.rightHandLandmarks) { // Gesto de duas mãos: Resize
      ...
      logDebug(`Forma ${shape.id} Gesto Resize Check: lIdxCurl: ${lIdxCurl} (Y8:${shape.leftHandLandmarks[8].y.toFixed(2)} > Y6:${shape.leftHandLandmarks[6].y.toFixed(2)}), rIdxCurl: ${rIdxCurl} (Y8:${shape.rightHandLandmarks[8].y.toFixed(2)} > Y6:${shape.rightHandLandmarks[6].y.toFixed(2)})`);
      ...
      if (lIdxCurl && rIdxCurl) { // Se ambos indicadores curvados (simulando agarrar)
        ...
        logDebug(`Forma ${shape.id}: Gesto Resize ATIVO. Dist: ${dist.toFixed(1)}, NormDist: ${normDist.toFixed(2)}, Novo Raio: ${shape.radius.toFixed(1)}`);
    ...
    if (!gestureProcessed && shape.leftHandLandmarks) { // Gesto mão esquerda: Mudar lados (Pinch)
      ...
      logDebug(`Forma ${shape.id} Gesto Sides Check: PinchDist: ${pinchDist.toFixed(1)}, isTouchingCircle: ${isTouching} (PinchXY: ${pinchCanvasX.toFixed(0)},${pinchCanvasY.toFixed(0)} vs ShapeCXY: ${shape.centerX.toFixed(0)},${shape.centerY.toFixed(0)} R:${shape.radius.toFixed(0)})`);
      ...
      if (isTouching) {
        ...
        logDebug(`Forma ${shape.id}: Gesto Sides ATIVO. PinchDist: ${pinchDist.toFixed(1)}, Novos Lados: ${newSides}`);
    ...
    if (!gestureProcessed && shape.rightHandLandmarks) { // Gesto mão direita: Liquify (se nenhuma outra prioridade)
      ...
      logDebug(`Forma ${shape.id}: Gesto Liquify ATIVO (por padrão com mão direita).`);
    }
    ...
    if (currentGesture) {
        logDebug(`Forma ${shape.id}: Gesto FINALMENTE definido como: ${currentGesture}`);
    } else if (shape.leftHandLandmarks || shape.rightHandLandmarks) {
        logDebug(`Forma ${shape.id}: NENHUM gesto específico definido, apesar de haver mãos.`);
    }
  });
...
function setupEventListeners() {
    ...
    // Event listener for the new Info HUD button
    if (infoHudButton && hudElement) {
        infoHudButton.addEventListener('click', () => {
            ...
            if (isHidden) {
                ...
                updateHUD(); // Chama updateHUD para redesenhar o conteúdo se ele estava oculto
            }
        });
        // Definir estado inicial do botão ao carregar a página
        // O HUD é visível por padrão (sem a classe 'hidden' no HTML inicial)
        // Portanto, o botão deve refletir isso.
        if (hudElement.classList.contains('hidden')) { // Embora padrão seja visível, checa por segurança
            infoHudButton.textContent = "ℹ️ Mostrar HUD";
            infoHudButton.classList.remove('active');
        } else {
            infoHudButton.textContent = "ℹ️ Ocultar HUD";
    ...
    if (oscConfigButton && oscConfigModal) {
        oscConfigButton.addEventListener('click', () => { oscHostInput.value = OSC_HOST; oscPortInput.value = OSC_PORT; oscConfigModal.style.display = 'flex'; });
    }
    ...
    // if (oscPanelButton) oscPanelButton.addEventListener('click', () => {if(oscControlModal) oscControlModal.style.display = 'flex'}); // oscPanelButton might be removed
    ...
    // if (operationModeButton) operationModeButton.addEventListener('click', toggleOperationMode); // Might be removed
    ...
    // if (openOutputPopupButton) openOutputPopupButton.addEventListener('click', openPopup); // Might be removed
    // if (resetMidiButton) resetMidiButton.addEventListener('click', resetMidiSystem); // Might be removed
    // if (scaleCycleButton) scaleCycleButton.addEventListener('click', cycleScale); // Might be removed
...
function updateHUD() {
  ...
  // Se o HUD estiver explicitamente marcado como oculto:
  if (hudElement.classList.contains('hidden')) {
    // Opcionalmente, podemos limpar o conteúdo se desejado quando oculto,
    // ou simplesmente não fazer nada e deixar o conteúdo antigo lá.
    // Para este caso, vamos limpar para consistência com o comportamento anterior.
    let textSpan = hudElement.querySelector('span#hudTextContent');
    if (textSpan) {
        textSpan.innerHTML = ""; // Limpa o conteúdo
    }
    return; // Não prossegue com a atualização do conteúdo visível.
  }

  // Se chegou aqui, o hudElement existe e NÃO está com a classe 'hidden'.
  // Então, procedemos com a atualização normal do conteúdo do HUD.
  let txt = "";
  ...
  // O comentário sobre o 'else' removido sugere que o problema era um 'else' solto aqui.
...
function saveAllPersistentSettings(){savePersistentSetting('operationMode',operationMode);savePersistentSetting('midiEnabled',midiEnabled);/*...more...*/console.log("Configs V35 salvas.");}
function loadAllPersistentSettings(){operationMode=loadPersistentSetting('operationMode','two_persons');midiEnabled=loadPersistentSetting('midiEnabled',true);/*...more...*/loadOscSettings();loadArpeggioSettings();console.log("Configs V35 carregadas.");return{savedMidiOutputId:loadPersistentSetting('midiOutputId',null),savedMidiInputId:loadPersistentSetting('midiInputId',null)};}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    loadTheme();
    initPresetManager();
    setupEventListeners(); // This now includes the listener for infoHudButton
    ...
    initMidi().then(async () => {
        ...
        if (!mediaStream) { populateCameraSelect().then(() => initializeCamera(currentCameraDeviceId)); }
    });
    ...
    // ... (Most UI state applications as in v41) ...
    if(hudElement) hudElement.classList.remove('hidden'); // Ensure HUD is visible by default

    updateHUD();
    sendAllGlobalStatesOSC();

    if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas.\n`;
    
    console.log("Iniciando loop de animação e finalizando DOMContentLoaded.");
    animationLoop(); // Inicia o loop de animação
});
...
function animationLoop() {
  ...
  if (cameraError && !gestureSimulationActive) {
    drawFallbackAnimation();
    // updateHUD(); // updateHUD é chamado por onResults ou drawFallbackAnimation se a câmera estiver onFrame
  }
  // Se a câmera estiver OK, onResults -> drawShape já lida com o redesenho.
  // Se cameraError for true, e camera.onFrame não estiver sendo chamado,
  // drawFallbackAnimation aqui garante que algo seja mostrado.
}
