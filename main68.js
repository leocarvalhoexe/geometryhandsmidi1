...
function setupEventListeners() {
    ...
    if (internalAudioToggleButton) {
        internalAudioToggleButton.addEventListener("click", async () => {
            // Lógica do clique para internalAudioToggleButton (se houver alguma além de configurar o synth depois)
            // Por exemplo, garantir que o AudioContext está rodando.
            if (!audioCtx) {
            ...
            if (_internalAudioEnabledMaster) {
                if (!simpleSynth && audioCtx) {
                    ...
                    // Reaplicar configurações salvas ao simpleSynth, se necessário
...
function saveAllPersistentSettings(){
  ...
  console.log("Configs V68 salvas no localStorage."); // Updated version
}
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main68.js..."); // Updated version
    console.log("DOM Carregado. Iniciando main68.js (v68)..."); // Updated version
    ...
    if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas (v68).\n`; // Updated version
    loadPlayPauseState();
