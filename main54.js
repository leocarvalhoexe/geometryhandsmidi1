...
// simpleSynth e audioCtx são gerenciados e obtidos de synth54.js
...
class Shape {
  constructor(id, midiChannel) {
    ...
    this.timingVariationFactor = 1.0; // v54 - For geometric distortion timing effect
  }
}
...
const PRESETS_STORAGE_KEY = 'midiShapePresetsV54'; // v54 update
let shapePresets = {};
const APP_SETTINGS_KEY = 'midiShapeManipulatorV54Settings'; // v54 update
const ARPEGGIO_SETTINGS_KEY = 'arpeggioSettingsV54'; // v54 update
const CAMERA_DEVICE_ID_KEY = 'midiShapeCameraDeviceIdV54'; // v54 update
const NOTE_MODE_STORAGE_KEY = 'midiShapeNoteModeV54'; // v54 add
...
// === END V48/V49/V52 Elements ===

// V54: Note Mode Selector Element
let noteModeSelect = document.getElementById('noteModeSelect');
...
const NOTE_MODES = { // v54: Changed to an object for easier value access
    SEQUENTIAL: 'SEQUENTIAL',
    ARPEGGIO: 'ARPEGGIO',
    CHORD: 'CHORD',
    RANDOM_WALK: 'RANDOM_WALK'
};
let currentNoteMode = NOTE_MODES.SEQUENTIAL; // v54: Initialize with default
...
function drawShape(shape, isPulsing, pulseValue) {
  ...
  // v54: Update timingVariationFactor based on distortion
  const MAX_TIMING_VARIATION = 0.2; // Max 20% variation
  // Increase factor for more distortion (slower), decrease for less (faster)
  // This makes distorted notes slightly delayed, creating a "drag" or "swing"
  shape.timingVariationFactor = 1.0 + (normDistortion * MAX_TIMING_VARIATION);
  // To make it potentially faster as well, one could use:
  // shape.timingVariationFactor = 1.0 + ((normDistortion - 0.5) * 2 * MAX_TIMING_VARIATION);
  // This would make 0 distortion = 1.0 - MAX_TIMING_VARIATION (faster), max distortion = 1.0 + MAX_TIMING_VARIATION (slower)
  // For now, keeping it simple: more distortion = more delay/drag.
...
function processShapeNotes(shape, isPulsing, pulseValue) {
    ...
    // v54: Apply timingVariationFactor to noteInterval
    ...
    if (canPlayNonArp || canPlayArp) {
        ...
        switch (currentNoteMode) {
            case NOTE_MODES.SEQUENTIAL: // v54
                    ...
                    if (!staccatoModeActive) {
                        Object.keys(shape.activeMidiNotes).forEach(k => {
                            if (k.startsWith(`seq_${shape.id}_`) && shape.activeMidiNotes[k]?.playing) {
                                ...
                                delete shape.activeMidiNotes[k];
                            }
                        });
                    }
                    for (let i = 0; i < shape.sides; i++) {
            ...
            case NOTE_MODES.RANDOM_WALK: // v54
                if (canPlayNonArp) {
                    shape.currentEdgeIndex += Math.floor(Math.random() * 3) - 1;
                    const scaleNoteCount = SCALES[currentScaleName].notes.length * 2; // Using double the scale length for more variety
        ...
        if (notePlayed && notesToPlay.length > 0) {
            ...
            notesToPlay.forEach((noteObjOrNote, index) => {
                ...
                if (typeof noteObjOrNote === 'object' && noteObjOrNote.hasOwnProperty('note')) {
                    noteToPlay = noteObjOrNote.note;
                    vertexIdxForKey = noteObjOrNote.vertexIndex;
                } else {
                    noteToPlay = noteObjOrNote;
                    vertexIdxForKey = edgeIdx;
                }

                let key;
                if (currentNoteMode === NOTE_MODES.SEQUENTIAL) {
                    key = `seq_${shape.id}_${vertexIdxForKey}`;
                } else if (currentNoteMode === NOTE_MODES.ARPEGGIO) {
                    key = `arp_${shape.id}_${vertexIdxForKey}`;
                } else if (currentNoteMode === NOTE_MODES.CHORD) {
                    key = `chord_${shape.id}_${noteToPlay}_${index}`;
                } else {
                    key = `${vertexIdxForKey}_0`;
                }
                ...
                shape.activeMidiNotes[key] = {
                    ...
                    isArpeggioNote: currentNoteMode === NOTE_MODES.ARPEGGIO, // v54
                    isSequentialNote: currentNoteMode === NOTE_MODES.SEQUENTIAL // v54
                };
...
function setupEventListeners() {
    ...
    // V54: Listener para Note Mode Select
    if (noteModeSelect) {
        noteModeSelect.addEventListener('change', (e) => {
            if (spectatorModeActive) return;
            currentNoteMode = e.target.value;
            turnOffAllActiveNotes(); // Turn off notes when changing mode to avoid stuck notes
            savePersistentSetting(NOTE_MODE_STORAGE_KEY, currentNoteMode);
            updateHUD();
            sendOSCMessage('/global/state/noteMode', currentNoteMode);
            console.log(`Note Mode alterado para: ${currentNoteMode}`);
        });
    }
...
function initSynthControlsSidebar() {
    ...
    if (scBPMSlider) {
        scBPMSlider.addEventListener('input', (e) => {
            ...
            updateBPMValues(arpeggioBPM); // Atualiza todos os lugares que mostram BPM e o noteInterval
    ...
    // Bloco problemático removido. A lógica correta está abaixo.
...
function updateHUD() {
  ...
  txt += `&nbsp;&nbsp;Escala:${SCALES[currentScaleName].name} Modo:${currentNoteMode} Acorde:${chordMode} Oper:${operationMode==='one_person'?'1P':'2P'}<br>`; // v54 Note Mode display
  if (currentNoteMode === NOTE_MODES.ARPEGGIO) txt += `&nbsp;&nbsp;Arp: ${currentArpeggioStyle} BPM:${arpeggioBPM.toFixed(0)}${externalBPM!==null?'(Ext)':''} Idx:${shapes.map(s=>s.currentEdgeIndex).join('/')}<br>`; // v54
...
function saveAllPersistentSettings(){
  ...
  savePersistentSetting(NOTE_MODE_STORAGE_KEY, currentNoteMode); // V54 save note mode
  ...
  console.log("Configs V54 salvas no localStorage."); // V54 update
}
...
function loadAllPersistentSettings(){
  ...
  currentNoteMode = loadPersistentSetting(NOTE_MODE_STORAGE_KEY, NOTE_MODES.SEQUENTIAL); // V54 load note mode

  if (internalAudioToggleButton) {
      internalAudioToggleButton.textContent = internalAudioEnabled ? "🔊 Áudio ON" : "🔊 Áudio OFF";
      internalAudioToggleButton.classList.toggle('active', internalAudioEnabled);
  }
  // V54: Apply loaded note mode to the selector
  if (noteModeSelect) {
    noteModeSelect.value = currentNoteMode;
  }
  ...
  loadOscSettings();
  loadArpeggioSettings(); // This will also call updateBPMValues
  // const isSynthPanelHidden = loadPersistentSetting('synthPanelHidden', false); // Carregado em initSynthControlsSidebar

  console.log("Configs V54 carregadas do localStorage."); // V54 update
  return {
...
function loadArpeggioSettings(){
    try{
        ...
        if(s){
            ...
            // externalBPM não é persistido intencionalmente ou é resetado
        }
    }catch(e){}
    if(arpeggioStyleSelect) arpeggioStyleSelect.value = currentArpeggioStyle;

    updateBPMValues(arpeggioBPM); // V52 - Atualiza todos os controles de BPM e Intervalo
    // As linhas abaixo são agora tratadas por updateBPMValues e updateNoteIntervalValues
}

function updateBPMValues(newBPM) {
    // v54: Ensure BPM is within new 3-900 range if called directly, though sliders should enforce this.
    arpeggioBPM = Math.max(3, Math.min(900, parseInt(newBPM, 10)));
    ...
    // Update noteInterval slider based on new BPM, respecting its own min/max
    if (noteIntervalSlider) {
        noteIntervalSlider.value = Math.max(parseInt(noteIntervalSlider.min), Math.min(parseInt(noteIntervalSlider.max), noteInterval));
    }
    if (noteIntervalValueSpan) noteIntervalValueSpan.textContent = noteIntervalSlider.value; // Display the potentially clamped value
}

function updateNoteIntervalValues(newInterval) {
    noteInterval = parseInt(newInterval, 10);
    // Calculate BPM based on new interval, then clamp BPM to 3-900 range
    let calculatedBPM = Math.round(60000 / noteInterval);
    arpeggioBPM = Math.max(3, Math.min(900, calculatedBPM));

    // Recalculate noteInterval based on the clamped BPM to ensure consistency
    noteInterval = Math.round(60000 / arpeggioBPM);


    if (noteIntervalSlider) noteIntervalSlider.value = noteInterval; // Value is now consistent with clamped BPM
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main54.js..."); // V54 update
    console.log("DOM Carregado. Iniciando main54.js..."); // V54 update
    detectPlatform();
    ...
    const firstGestureHandler = () => {
        console.log("Primeiro gesto detectado, tentando inicializar o áudio via synth54.js..."); // V54 update
        if (typeof initAudioContextOnGesture === "function") {
            ...
            if (audioReady) {
                ...
                console.warn("initAudioContextOnGesture() de synth54.js não retornou sucesso. O áudio pode não estar pronto."); // V54 update
            }
        } else {
            console.error("initAudioContextOnGesture não está definida globalmente (esperada de synth54.js)."); // V54 update
        }
    };
    ...
    currentCameraDeviceId = localStorage.getItem(CAMERA_DEVICE_ID_KEY) || null; // v54 key
  ...
  console.log("Iniciando loop de animação (v54) e finalizando DOMContentLoaded."); // V54 update
    animationLoop();
});
...
function startAudioRecording() {
    ...
    try {
        ...
        currentSimpleSynth.masterGainNode.connect(destinationNode); // Conecta a saída principal do synth ao nó de destino da gravação

        // Verifica os codecs suportados
        ...
        if (!MediaRecorder.isTypeSupported(options.mimeType)) {
            ...
            if (!MediaRecorder.isTypeSupported(options.mimeType)) {
                ...
                options.mimeType = 'audio/webm'; // Fallback para webm genérico
                 if (!MediaRecorder.isTypeSupported(options.mimeType)) {
                    ...
                    currentSimpleSynth.masterGainNode.disconnect(destinationNode); // Limpa a conexão
                    return;
                 }
            }
        }
        ...
        mediaRecorder.onstop = () => {
            // A lógica de UI para 'stop' é tratada em stopAudioRecording() para centralizar
            // Mas precisamos garantir que o estado seja atualizado se o stop for chamado programaticamente
            // ou por outros meios que não o botão.
            if (isAudioRecording) { // Evita chamadas múltiplas se já parado pelo botão
            ...
            // Desconecta o nó de destino para evitar que continue processando desnecessariamente
            // É importante fazer isso APÓS a gravação parar e os dados serem coletados.
            try {
        ...
        mediaRecorder.onerror = (event) => {
            ...
            stopAudioRecording(); // Tenta parar de forma limpa
        };
...
function stopAudioRecording() {
    if (mediaRecorder && (mediaRecorder.state === "recording" || mediaRecorder.state === "paused")) {
        mediaRecorder.stop(); // Isso vai disparar o mediaRecorder.onstop
    }
    // A UI é atualizada principalmente pelo onstop, mas garantimos alguns estados aqui
    isAudioRecording = false;
    isAudioPaused = false;
    if (recordAudioButton) recordAudioButton.textContent = "⏺️ Gravar Áudio";
    if (recordAudioButton) recordAudioButton.classList.remove('active');
...
function saveRecordedAudio() {
    ...
    let mimeType = 'audio/webm; codecs=opus'; // Padrão
    if (mediaRecorder && mediaRecorder.mimeType) { // Usa o mimeType real da gravação se disponível
        mimeType = mediaRecorder.mimeType;
    } else if (audioChunks.length > 0 && audioChunks[0].type && MediaRecorder.isTypeSupported(audioChunks[0].type)) {
        mimeType = audioChunks[0].type; // Tenta pegar do primeiro chunk
    }
