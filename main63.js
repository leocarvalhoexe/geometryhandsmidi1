...
// V63: Arpeggiator Variation DOM Elements
const arpRandomnessSlider = document.getElementById('arpRandomnessSlider');
const arpRandomnessValue = document.getElementById('arpRandomnessValue');
const arpSwingSlider = document.getElementById('arpSwingSlider');
const arpSwingValue = document.getElementById('arpSwingValue');
const arpGhostNoteChanceSlider = document.getElementById('arpGhostNoteChanceSlider');
const arpGhostNoteChanceValue = document.getElementById('arpGhostNoteChanceValue');
...
const GESTURE_SOURCES = {
    ...
    // ROTATION_SPEED: "Velocidade de Rotação" // Implementar se/quando houver
};
...
function processShapeNotes(shape, isPulsing, pulseValue) {
    ...
    if (currentNoteMode === 'ARPEGGIO' && arpSwing > 0) {
        ...
        // Simple two-step swing: long-short or short-long
...
function setupEventListeners() {
    ...
    // V63: Arpeggiator Variation Listeners
    if (arpRandomnessSlider) arpRandomnessSlider.addEventListener('input', (e) => { if(spectatorModeActive) return; arpRandomness = parseInt(e.target.value); if(arpRandomnessValue) arpRandomnessValue.textContent = arpRandomness; saveArpeggioSettings(); updateHUD(); sendOSCMessage('/global/state/arpRandomness', arpRandomness); });
    if (arpSwingSlider) arpSwingSlider.addEventListener('input', (e) => { if(spectatorModeActive) return; arpSwing = parseInt(e.target.value); if(arpSwingValue) arpSwingValue.textContent = arpSwing; saveArpeggioSettings(); updateHUD(); sendOSCMessage('/global/state/arpSwing', arpSwing); });
    if (arpGhostNoteChanceSlider) arpGhostNoteChanceSlider.addEventListener('input', (e) => { if(spectatorModeActive) return; arpGhostNoteChance = parseInt(e.target.value); if(arpGhostNoteChanceValue) arpGhostNoteChanceValue.textContent = arpGhostNoteChance; saveArpeggioSettings(); updateHUD(); sendOSCMessage('/global/state/arpGhostNoteChance', arpGhostNoteChance); });
...
function loadArpeggioSettings(){
    ...
    updateBPMValues(arpeggioBPM); // This also updates noteInterval display

    // V63: Update UI for Arpeggiator Variations
    if(arpRandomnessSlider) arpRandomnessSlider.value = arpRandomness;
    if(arpRandomnessValue) arpRandomnessValue.textContent = arpRandomness;
    if(arpSwingSlider) arpSwingSlider.value = arpSwing;
    if(arpSwingValue) arpSwingValue.textContent = arpSwing;
    if(arpGhostNoteChanceSlider) arpGhostNoteChanceSlider.value = arpGhostNoteChance;
    if(arpGhostNoteChanceValue) arpGhostNoteChanceValue.textContent = arpGhostNoteChance;
}
