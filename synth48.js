// ==========================================================================
// SYNTHESIZER MODULE v48 - synth48.js
// ==========================================================================
...
class SimpleSynth {
  constructor(audioContext) {
    ...
    console.log("SimpleSynth v48 inicializado com AudioContext, ADSR padrão e DistortionNode."); // v48 Update
  }
  ...
  noteOn(midiNote, velocity = 127) {
    if (this.audioCtx.state === 'suspended') {
      this.audioCtx.resume().then(() => {
        console.log("AudioContext resumed by noteOn in synth48.js"); // v48 Update
        this._playNote(midiNote, velocity);
      }).catch(e => console.error("Error resuming AudioContext in synth48.js:", e)); // v48 Update
      return;
    }
  ...
  allNotesOff() {
    console.log("Synth v48 All Notes Off (ADSR aware)"); // v48 Update
    const now = this.audioCtx.currentTime;
  ...
  if (!audioCtx) {
    try {
      ...
      if (audioCtx.state === 'suspended') {
        console.log("AudioContext (v48) está suspenso. Requer interação do usuário para iniciar."); // v48 Update
      }
      simpleSynth = new SimpleSynth(audioCtx);
      console.log("AudioContext e SimpleSynth (v48) inicializados."); // v48 Update
      return true;
    } catch (e) {
      console.error("Web Audio API não é suportada neste navegador (v48).", e); // v48 Update
      return false;
    }
  }
  return true;
...
function initAudioContextOnGesture() {
  if (!audioCtx) {
    if (!initAudioContext()) {
        console.error("Falha ao inicializar AudioContext no gesto (v48)."); // v48 Update
        return false;
    }
  }
  ...
  if (audioCtx && audioCtx.state === 'suspended') {
    audioCtx.resume().then(() => {
      console.log('AudioContext (v48) resumido com sucesso por gesto do usuário!'); // v48 Update
      if (!simpleSynth) {
          simpleSynth = new SimpleSynth(audioCtx);
          console.log("SimpleSynth (v48) instanciado após resumo do AudioContext."); // v48 Update
      }
      ...
      resumed = true;
        ...
        console.error('Erro ao resumir AudioContext (v48):', e); // v48 Update
        resumed = false;
    });
  } else if (audioCtx && audioCtx.state === 'running') {
    console.log('AudioContext (v48) já está rodando.'); // v48 Update
    resumed = true;
  } else {
    console.warn('AudioContext (v48) não está suspenso, mas também não está rodando, ou não foi inicializado.'); // v48 Update
    resumed = false;
  }
  return resumed;
}
...
function setInternalAudioEnabledState(enabled) {
    ...
    console.log(`Synth v48 _internalAudioEnabledMaster state set to: ${_internalAudioEnabledMaster}`); // v48 Update
}
...
console.log("synth48.js carregado."); // v48 Update
