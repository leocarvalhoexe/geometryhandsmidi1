// ==========================================================================
// SYNTHESIZER MODULE v62 - synth62.js
// ==========================================================================

// audioCtx e simpleSynth são agora gerenciados em main62.js
// _internalAudioEnabledMaster também é gerenciado em main62.js

const VALID_WAVEFORMS_V62 = ['sine', 'square', 'sawtooth', 'triangle', 'noise', 'pulse']; // 'custom' removido por enquanto, pois não estava implementado
...
class SimpleSynth {
  constructor(audioContext) {
    ...
    // V62: Buffer para a forma de onda 'noise'
    this.noiseBuffer = null;
    if (this.audioCtx) { // Garante que audioCtx existe antes de criar o buffer
        this._createNoiseBuffer();
    }


    console.log("SimpleSynth v62 inicializado."); // ATUALIZADO
  }

  // V62: Método para criar o buffer de ruído
  _createNoiseBuffer() {
    if (!this.audioCtx) {
        console.warn("AudioContext não disponível para criar noise buffer.");
        return;
    }
  ...
  setLfoPitchDepth(depth) {
    if (this.lfoGainPitch) {
      const clampedDepth = Math.max(0, Math.min(100, parseFloat(depth))); // Ajustado para Hz
  ...
  setLfoFilterDepth(depth) {
    if (this.lfoGainFilter) {
      const clampedDepth = Math.max(0, Math.min(10000, parseFloat(depth))); // Ajustado para Hz
  ...
  setWaveform(newWaveform) {
    if (VALID_WAVEFORMS_V62.includes(newWaveform)) { // V62: Usa nova lista
  ...
  _playNote(midiNote, velocity) {
    ...
    // Para 'noise', a frequência não é diretamente usada, mas mantemos o cálculo para consistência e possível uso futuro.
    if (freq <= 0 && this.waveform !== 'noise') { // V62: Permite freq 0 para 'noise'
        console.warn(`Frequência inválida (${freq}Hz) para MIDI note ${midiNote} com waveform ${this.waveform}. Nota não será tocada.`);
        return;
    }
    ...
    if (this.oscillators[midiNote]) {
        ...
        try {
            ...
            // V62: Desconectar LFO do pitch APENAS se não for 'noise'
    ...
    let osc; // V62: Pode ser OscillatorNode ou BufferSourceNode
    const gainNode = this.audioCtx.createGain();
    const now = this.audioCtx.currentTime;

    if (this.waveform === 'noise') {
        if (!this.noiseBuffer) this._createNoiseBuffer(); // Garante que o buffer existe
        if (!this.noiseBuffer) { // Se ainda não existir (ex: audioCtx não estava pronto antes)
            console.error("Buffer de ruído não pôde ser criado. Impossível tocar nota de ruído.");
            return;
        }
        osc = this.audioCtx.createBufferSource();
        osc.buffer = this.noiseBuffer;
        osc.loop = true;
        // LFO de pitch não se aplica diretamente a um BufferSource de ruído da mesma forma
    } else if (this.waveform === 'pulse') {
        osc = this.audioCtx.createOscillator();
        // Cria uma onda de pulso com ciclo de trabalho de 25% (exemplo)
        // Para uma onda de pulso que vai de 0 a 1, com duty cycle D:
        // Real (cos): [0, D, 0, D/2, 0, D/3, ...] (simplificado, não ideal para Fourier direto)
        // Imag (sin): [0, (1-cos(2*pi*D*k))/(k*pi), ...]
        // A forma mais simples de obter um pulso é usar um oscilador 'square' e, se necessário,
        // modular sua amplitude ou usar um WaveShaper para alterar o duty cycle.
        // Por ora, vamos usar createPeriodicWave para uma forma de pulso simples.
        // Coeficientes para algo que soe como pulso (não perfeitamente quadrado):
        // Exemplo para duty ~25%: forte fundamental, alguns harmônicos.
        const realCoeffs = new Float32Array([0, 0.6, 0.4, 0.2, 0.1, 0.05]); // Componente DC, fundamental, harmônicos
        const imagCoeffs = new Float32Array(realCoeffs.length).fill(0); // Sem componentes de seno para uma onda simétrica
        // Para um pulso mais "quadrado", os coeficientes de seno seriam importantes.
        // Exemplo alternativo para um pulso mais rico (pode precisar de disableNormalization: false):
        // const real = new Float32Array([0, 1, 0.8, 0.6, 0.4, 0.2]);
        // const imag = new Float32Array([0, 0, 0.1, 0.2, 0.3, 0.4]);

        try {
            const periodicWave = this.audioCtx.createPeriodicWave(realCoeffs, imagCoeffs, { disableNormalization: true });
            osc.setPeriodicWave(periodicWave);
        } catch (e) {
            console.warn("Falha ao criar PeriodicWave para 'pulse', usando 'square' como fallback.", e);
        ...
        osc = this.audioCtx.createOscillator();
        osc.type = this.waveform; // Garante que 'custom' não quebre se não implementado
        osc.frequency.setValueAtTime(freq, now);
        if (this.lfoGainPitch) this.lfoGainPitch.connect(osc.frequency);
    }
    ...
    osc.start(now);
    this.oscillators[midiNote] = { osc, gainNode, type: this.waveform }; // V62: Armazena o tipo
  }
  ...
  noteOff(midiNote) {
    ...
    if (this.oscillators[midiNote]) {
      const { osc, gainNode, type } = this.oscillators[midiNote]; // V62: Pega o tipo
      const now = this.audioCtx.currentTime;
  ...
  allNotesOff() {
    console.log("SimpleSynth v62 All Notes Off (ADSR aware)"); // ATUALIZADO
    const now = this.audioCtx.currentTime;
...
console.log("synth62.js carregado e pronto para ser instanciado por main62.js."); // ATUALIZADO
