// ==========================================================================
// MIDI SHAPE MANIPULATOR v56 - main56.js
// ==========================================================================

// simpleSynth e audioCtx são gerenciados e obtidos de synth53.js (ou versão compatível)
...
class Shape {
  constructor(id, midiChannel) {
    ...
    // v56 - Para gestos avançados
    this.previousFrameRadius = this.radius;
    this.liquifyIntensityPeaked = false;
    this.nextChordShouldBeInverted = false;
    this.currentInversionLevel = 0; // 0: fundamental, 1: 1ª inv, 2: 2ª inv
    this.fxBoostFactor = 1.0;
    this.transpositionGestureState = { raised: false, lowered: false, neutral: true }; // Para transposição
    this.lastResizeTimeForFx = performance.now(); // v56 Inicialização para FX Boost
    this.lastSignificantChangeTime = performance.now(); // v56 Para pausas dinâmicas por inatividade
  }
}
...
// v56 - Para gestos avançados
let globalKeyTransposeSemits = 0; // Transposição global em semitons
const MAX_GLOBAL_TRANSPOSE = 6; // Limite para transposição (para cima ou para baixo)
const TRANSPOSE_GESTURE_THRESHOLD_Y_FACTOR = 0.25; // 25% da altura da tela para gatilho de transposição
const FX_BOOST_RADIUS_CHANGE_SPEED_THRESHOLD = 7.0; // Quão rápido o raio deve mudar para boost de FX
const FX_BOOST_MULTIPLIER = 1.5; // Multiplicador para reverb/delay
const LIQUIFY_INTENSITY_THRESHOLD_FOR_INVERSION = 100; // Valor de modWheel (0-127) para armar inversão

// v56 - Constantes para Pausas Intencionais Dinâmicas
const THRESHOLD_SIDES_FOR_PAUSE = 4; // Menos que X lados pode pausar
const THRESHOLD_RADIUS_FOR_PAUSE = 40; // Menos que X de raio pode pausar
const IDLE_TIMEOUT_FOR_PAUSE_MS = 3000; // 3 segundos de inatividade para pausar
...
function updateMusicalStructureAndTiming() {
    if (spectatorModeActive && !isRecordingOSC && !isPlayingOSCLoop) return; // Permitir que OSC Rec/Play funcionem em modo espectador

    const now = performance.now();
    if (lastBeatTime === 0) lastBeatTime = now;

    // Avança o contador de batidas principal e processa a cadência harmônica
    if (now - lastBeatTime >= noteInterval) {
        masterBeatCounter++;
        lastBeatTime = now;

        if (masterBeatCounter % BEATS_PER_CHORD === 0) {
            if (!spectatorModeActive) advanceHarmonicProgression(); // Avança o acorde da cadência
            ...
            if (automaticNoteModeChangeEnabled && !spectatorModeActive) {
                ...
                if (measuresCounterForNoteMode >= NOTE_MODE_CHANGE_INTERVAL_MEASURES) {
                    ...
                    updateHUD(); // Atualiza HUD para refletir mudança de modo
                }
            }
        }
    }
}
// === FIM FUNÇÕES DA CADÊNCIA HARMÔNICA v56 ===

function processShapeNotes(shape, isPulsing, pulseValue) {
    // Se o modo PAUSE foi definido pela alternância automática, respeitar isso.
    if (currentNoteMode === 'PAUSE' && automaticNoteModeChangeEnabled) {
        if (performance.now() - shape.lastNotePlayedTime > noteInterval) { // Para que a pausa "consuma" um tempo de nota
    ...
    // v56 - Lógica para Pausas Intencionais Dinâmicas
    const now = performance.now();
    let isDynamicallyPaused = false;
    if (shape.sides < THRESHOLD_SIDES_FOR_PAUSE && shape.sides !== 100) { // Círculo (100) não deve pausar por "poucos lados"
    ...
    if (isDynamicallyPaused) {
        if (now - shape.lastNotePlayedTime > noteInterval) { // Para que a pausa "consuma" um tempo de nota
            turnOffAllActiveNotesForShape(shape);
            shape.lastNotePlayedTime = now; // Atualiza o tempo mesmo em pausa para evitar rajada de notas ao retomar
        }
        return; // Interrompe a geração de novas notas para esta forma
    }
    // Fim Pausas Intencionais Dinâmicas v56
    ...
    if (canPlayNonArp || canPlayArp) {
        ...
        switch (currentNoteMode) {
            case 'SEQUENTIAL':
                if (canPlayNonArp && shape.sides > 0) {
                    // Desligar notas sequenciais anteriores se não estiverem em modo staccato
                    if (!staccatoModeActive) {
                        Object.keys(shape.activeMidiNotes).forEach(k => {
                            if (k.startsWith(`seq_${shape.id}_`) && shape.activeMidiNotes[k]?.playing) {
                                sendMidiNoteOff(shape.activeMidiNotes[k].note, shape.midiChannel, shape.id + 1);
                                shape.activeMidiNotes[k].playing = false; // Marcar como não tocando
                                if (shape.activeMidiNotes[k].staccatoTimer) {
                                    clearTimeout(shape.activeMidiNotes[k].staccatoTimer);
                                }
                                delete shape.activeMidiNotes[k]; // Remover da lista de notas ativas
                            }
                        });
                    }

                    for (let i = 0; i < shape.sides; i++) {
                        notesToPlay.push({ note: getNoteInScale(i), vertexIndex: i });
                    }
                    notePlayed = true;
            ...
            case 'CHORD':
                if (canPlayNonArp) {
                    ...
                    if (edgeIdx < shape.sides) { // Ainda usamos edgeIdx para dar uma sensação de movimento/variação na forma
                        const currentCadenceChordName = currentHarmonicProgression[currentChordInProgressionIndex];
                        if (currentCadenceChordName) {
                            let numNotesInChord = 3; // Tríade padrão
                            if (shape.sides >= 4 && shape.sides < 7) numNotesInChord = 4; // Tocar com 7ª
                            else if (shape.sides >= 7) numNotesInChord = 5; // Tocar com 9ª (ou mais notas se getChordNotes suportar)
                            
                            // Usar uma oitava um pouco mais alta para os acordes da forma, para diferenciar da cadência base.
                            // A inversão pode ser controlada por gestos ou outro parâmetro no futuro.
                            // Por agora, a inversão pode ser ligada ao currentEdgeIndex para variar.
                            let inversionForShapeChord = shape.currentEdgeIndex % numNotesInChord; 
                            // Limitar a inversão para o que faz sentido (ex: max 2 para tríades, 3 para tétrades)
                            if (numNotesInChord === 3) inversionForShapeChord = inversionForShapeChord % 3;
                            if (numNotesInChord === 4) inversionForShapeChord = inversionForShapeChord % 4;
                            // Para 5 notas, a inversão além da 2a ou 3a pode não ser tão comum/útil, mas deixamos por enquanto.

                            // v56 - Aplicar inversão se nextChordShouldBeInverted estiver true
                            if (shape.nextChordShouldBeInverted) {
                                shape.currentInversionLevel = (shape.currentInversionLevel + 1) % numNotesInChord; // Cicla a inversão
                                shape.nextChordShouldBeInverted = false; // Consome a flag
                                logDebug(`Forma ${shape.id}: Aplicando inversão ${shape.currentInversionLevel}`);
                            }
                            // Se não for para inverter pelo gesto, usa a inversão baseada no edgeIndex.
                            // Caso contrário, usa a currentInversionLevel da forma, que foi alterada pelo gesto.
                            let finalInversionLevel = shape.nextChordShouldBeInverted ? shape.currentInversionLevel : inversionForShapeChord;


                            notesToPlay = getChordNotes(currentCadenceChordName, CADENCE_BASE_OCTAVE + 1, numNotesInChord, finalInversionLevel);
                            logDebug(`Forma ${shape.id} CHORD: ${currentCadenceChordName}, Lados: ${shape.sides} -> ${numNotesInChord} notas, Inversão Final: ${finalInversionLevel}, Notas: ${notesToPlay}`);

                            if (notesToPlay.length > 0) {
                                Object.values(shape.activeMidiNotes).forEach(ni => { 
                                    if (ni.playing) sendMidiNoteOff(ni.note, shape.midiChannel, shape.id + 1); 
                                    if (ni.staccatoTimer) clearTimeout(ni.staccatoTimer); 
                                });
                    ...
                    if(notePlayed) shape.lastNotePlayedTime = now;
                }
                break;
            case 'PAUSE':
                // Não toca notas novas. Desliga as existentes para esta forma.
                if (canPlayNonArp) { // Usar canPlayNonArp para que a pausa também siga o ritmo/intervalo
                    turnOffAllActiveNotesForShape(shape);
                    notePlayed = false; // Explicitamente não tocou
                    shape.lastNotePlayedTime = now; // Consome o tempo para que não toque imediatamente ao sair da pausa
                    logDebug(`Forma ${shape.id} em modo PAUSA.`);
                }
                break;
            case 'RANDOM_WALK':
                if (canPlayNonArp) {
                    shape.currentEdgeIndex += Math.floor(Math.random() * 3) - 1;
                    const scaleNoteCount = SCALES[currentScaleName].notes.length * 2; // Permite mais variedade
        ...
        if (notePlayed && notesToPlay.length > 0) {
            ...
            notesToPlay.forEach((noteObjOrNote, index) => {
                ...
                if (typeof noteObjOrNote === 'object' && noteObjOrNote.hasOwnProperty('note')) { // Para SEQUENTIAL
                    noteToPlay = noteObjOrNote.note;
                    vertexIdxForKey = noteObjOrNote.vertexIndex;
                } else { // Para outros modos (ARPEGGIO, CHORD, RANDOM_WALK)
                    noteToPlay = noteObjOrNote;
                    vertexIdxForKey = edgeIdx; // Usa o edgeIdx global para esses modos
                }

                let key;
                if (currentNoteMode === 'SEQUENTIAL') {
                    key = `seq_${shape.id}_${vertexIdxForKey}`;
                } else if (currentNoteMode === 'ARPEGGIO') {
                    key = `arp_${shape.id}_${vertexIdxForKey}`; // edgeIdx já está em vertexIdxForKey para Arpeggio
                } else if (currentNoteMode === 'CHORD') {
                    key = `chord_${shape.id}_${noteToPlay}_${index}`; // Chord pode ter múltiplas notas com o mesmo valor de nota base (edgeIdx)
                } else { // RANDOM_WALK e fallback
                    key = `${vertexIdxForKey}_0`; // edgeIdx já está em vertexIdxForKey para RandomWalk
                }
                ...
                shape.activeMidiNotes[key] = {
                    ...
                    isSequentialNote: currentNoteMode === 'SEQUENTIAL'
                };
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main56.js...");
  console.log("DOM Carregado. Iniciando main56.js (v56)..."); // Log de versão atualizado
    detectPlatform();
