// ==========================================================================
// MIDI SHAPE MANIPULATOR v42 - main42.js
// ==========================================================================
...
let handsInstance; let cameraUtil;
async function populateCameraSelect() { /* ... (implementation as in v41) ... */ }
async function initializeCamera(deviceId = null) { /* ... (implementation as in v41) ... */ }
...
function setupEventListeners() {
    ...
    if (infoHudButton && hudElement) {
        infoHudButton.addEventListener('click', () => {
            ...
            // Optionally, update button text/state if needed, e.g.,
            // infoHudButton.textContent = hudElement.classList.contains('hidden') ? "ℹ️ Mostrar HUD" : "ℹ️ Ocultar HUD";
            // For now, just toggling visibility is enough as per requirements.
        });
    }
...
function updateHUD() {
  if (!hudElement || hudElement.classList.contains('hidden')) { // Check if HUD is hidden
    if (hudElement && !hudElement.classList.contains('hidden')) { // Ensure it's not hidden before trying to clear
        let textSpan = hudElement.querySelector('span#hudTextContent');
        if (textSpan) textSpan.innerHTML = ""; // Clear content if hidden
    }
    return; // Don't update if hidden
  }
  // ... (rest of the updateHUD implementation as in v41)
  let txt = "";
  ...
  if (!spectatorModeActive && osc && osc.status() === OSC.STATUS.IS_OPEN && (now - lastOscSendTime > OSC_SEND_INTERVAL)) {
    lastOscSendTime = now;
    shapes.forEach(s => { const sid = s.id + 1; sendOSCMessage(`/forma/${sid}/radius`, parseFloat(s.radius.toFixed(2))); sendOSCMessage(`/forma/${sid}/sides`, s.sides); /* ... more OSC ... */ });
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Carregado. Iniciando main42.js..."); // Version Update
    detectPlatform();
    ...
    console.log("main42.js inicialização completa."); // Version Update
});
