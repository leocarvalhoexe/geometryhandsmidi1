...
function drawShape(targetCtx, shape, currentPulsedRadius, isPulsingActive, pulseCycleValue, currentDemoAngle, isDemoDistortingFlag) {
  ...
  for (let i = 0; i < currentDisplaySides; i++) {
    ...
    if (targetCtx === ctx && rightHandLandmarks && !demoModeActive && shape.isSelected) {
    ...
    if (targetCtx === ctx && midiEnabled && currentDisplaySides > 0) {
        const edgeIndex = i;
    ...
    if (i === 0) targetCtx.moveTo(finalX, finalY);
    else targetCtx.lineTo(finalX, finalY);
  }
  ...
  if (Object.keys(shape.activeNotes).length > 0) {
    ...
    } else if (targetCtx === ctx && (!midiEnabled || currentDisplaySides === 0)) { // If MIDI disabled or shape has no sides, turn off all its notes
...
function onResults(results) {
  ...
  // Clear canvas (hands will be drawn on a clean slate)
  if (ctx && cW && cH) {
    ctx.clearRect(0, 0, cW, cH); // Clear completely for hands
  }
  ...
  // Apply fade effect for the shape trail
  if (ctx && cW && cH) {
  ...
  for (const shape of shapes) {
    ...
    drawShape(ctx, shape, radiusToDraw, isShapePulsing, actualPulseCycleValue, demoAngle, demoIsDistorting && shape.isSelected);
  }
  ...
  if (outputPopupWindow && !outputPopupWindow.closed && popupCanvasCtx) {
    try {
      ...
      // Determine pulsing for shapes (reuse logic from main canvas drawing)
      ...
      if (currentOverallPulseActiveForPopup) {
        let tempPulseTime = performance.now() * 0.001; // Use a local calculation for safety
        actualPulseCycleValueForPopup = Math.sin(tempPulseTime * pulseFrequency * 2 * Math.PI);
      }

      // Draw shapes to popup
      for (const shape of shapes) {
        let radiusToDrawPopup = shape.radius;
        let isShapePulsingPopup = currentOverallPulseActiveForPopup && shape.isSelected;
        if (demoModeActive && demoIsPulsing && shape.isSelected) isShapePulsingPopup = true;

        if (isShapePulsingPopup) {
          let pulseRadiusModifierFactor = 0.25 * actualPulseCycleValueForPopup;
          radiusToDrawPopup = shape.radius * (1 + pulseRadiusModifierFactor);
          radiusToDrawPopup = Math.max(10, radiusToDrawPopup); // Ensure min radius
        }

        drawShape(popupCanvasCtx, shape, radiusToDrawPopup, isShapePulsingPopup, actualPulseCycleValueForPopup,
                  (demoModeActive && shape.isSelected ? demoAngle : 0),
                  (demoModeActive && demoIsDistorting && shape.isSelected));
      }
      }
    } catch (e) {
