// ==========================================================================
// MIDI SHAPE MANIPULATOR v61 - main61.js
// ==========================================================================
...
let shapePresets = {};
const APP_SETTINGS_KEY = 'midiShapeManipulatorV55Settings';
...
function processShapeNotes(shape, isPulsing, pulseValue) {
    ...
    // V61: Intervalo de tempo para notas de ARPEGGIO e SEQUENTIAL/CHORD/RANDOM_WALK
    // O arpejador usa seu próprio timer (lastArpeggioNotePlayedTime) e o noteInterval é baseado em arpeggioBPM.
    // Os outros modos (SEQUENTIAL, CHORD, RANDOM_WALK) usarão lastNotePlayedTime e o noteInterval global.
    const currentNoteInterval = (currentNoteMode === 'ARPEGGIO') ? (60000 / arpeggioBPM) : noteInterval;

    const canPlayArp = currentNoteMode === 'ARPEGGIO' && shape.sides > 0 && (now - shape.lastArpeggioNotePlayedTime > currentNoteInterval);
    const canPlayNonArp = currentNoteMode !== 'ARPEGGIO' && (now - shape.lastNotePlayedTime > currentNoteInterval);

    if (canPlayArp || canPlayNonArp) {
        let notesToPlay = []; // V61: Agora conterá no máximo UMA nota para os modos ARPEGGIO, SEQUENTIAL, CHORD
        let edgeIdx = shape.currentEdgeIndex; // Usado para ARPEGGIO e SEQUENTIAL
        let notePlayedThisTick = false;

        // V61: Limpar notas anteriores ANTES de determinar a próxima nota para os modos de nota única.
        // Para staccato, o timer existente cuidará do noteOff.
        // Para não-staccato, a nota anterior deve ser explicitamente desligada.
        if (!staccatoModeActive) {
            Object.keys(shape.activeMidiNotes).forEach(key => {
                const noteInfo = shape.activeMidiNotes[key];
                if (noteInfo && noteInfo.playing && !noteInfo.staccatoTimer) { // Só desliga se não for staccato já timerizado
                    sendMidiNoteOff(noteInfo.note, shape.midiChannel, shape.id + 1);
                    noteInfo.playing = false; // Marca como não tocando
                                        // Não deletar de activeMidiNotes aqui, a limpeza geral em drawShape fará isso.
                }
            });
        }
        // Se for staccato, a nota anterior pode continuar soando. Se a NOVA nota for a MESMA,
        // o timer staccato antigo será limpo e um novo iniciado.
        ...
        switch (currentNoteMode) {
            case 'SEQUENTIAL':
                if (canPlayNonArp && shape.sides > 0) {
                    ...
                    notesToPlay.push({ note: getNoteInScale(edgeIdx), vertexIndex: edgeIdx, isSequential: true });
            ...
            case 'ARPEGGIO':
                if (canPlayArp && shape.sides > 0) { // sides > 0 já estava, sides > 2 era para updown antigo
                    if (shape.sides < 1) break; // Evita divisão por zero ou loop infinito se sides for 0
                    ...
                    if (currentArpeggioStyle === "UP") {
                        ...
                        if (shape.sides === 1) {
                            shape.currentEdgeIndex = 0; // Com 1 lado, sempre toca ele
                        } else {
                            if (shape.arpeggioDirection === 1) { // Subindo
                                if (shape.currentEdgeIndex >= shape.sides - 1) {
                                    shape.currentEdgeIndex = Math.max(0, shape.sides - 1); // Fica no topo
                                    shape.arpeggioDirection = -1; // Muda para descer
                                    // Na próxima vez, vai decrementar de shape.sides - 1
                                } else {
                                    shape.currentEdgeIndex++;
                                }
                            } else { // Descendo (shape.arpeggioDirection === -1)
                                if (shape.currentEdgeIndex <= 0) {
                                    shape.currentEdgeIndex = 0; // Fica na base
                                    shape.arpeggioDirection = 1; // Muda para subir
                                     // Na próxima vez, vai incrementar de 0
                                } else {
                                    shape.currentEdgeIndex--;
                                }
                            }
                        }
                    } else if (currentArpeggioStyle === "RANDOM") {
                        shape.currentEdgeIndex = Math.floor(Math.random() * shape.sides);
                    }
                    edgeIdx = shape.currentEdgeIndex;
                    notesToPlay.push({ note: getNoteInScale(edgeIdx), vertexIndex: edgeIdx, isArpeggio: true });
                    notePlayedThisTick = true;
                    shape.lastArpeggioNotePlayedTime = now;
                }
                break;

            case 'CHORD': // V61: Modo CHORD agora toca notas do acorde sequencialmente
                if (canPlayNonArp && shape.sides > 0) {
                    const baseVertexIndex = shape.currentEdgeIndex;
                    const chordNotesDefinition = [0, 2, 4]; // Define as notas do acorde (ex: tônica, terça, quinta da escala)

                    // Avança para a próxima nota do acorde sequencial
                    const noteInChordToPlay = getNoteInScale(baseVertexIndex + chordNotesDefinition[shape.currentChordStepIndex]);

                    notesToPlay.push({
                        note: noteInChordToPlay,
                        vertexIndex: baseVertexIndex, // Vértice base do acorde
                        isChordNote: true,
                        chordPart: shape.currentChordStepIndex // Qual parte do acorde está tocando
                    });
                    ...
                    if (shape.currentChordStepIndex >= chordNotesDefinition.length) {
                        ...
                        shape.currentEdgeIndex = (shape.currentEdgeIndex + 1) % shape.sides;
                        // Opcional: stopAllNotesForShape(shape, true) se quiser silenciar tudo ao mudar de acorde base
                    }
                    notePlayedThisTick = true;
                    shape.lastNotePlayedTime = now;
                }
            ...
            case 'RANDOM_WALK':
                if (canPlayNonArp) {
                    // A lógica original do RANDOM_WALK já toca uma nota por vez.
                    // Apenas precisa garantir que use currentEdgeIndex para consistência, se aplicável,
                    // ou manter sua lógica de índice própria se for mais adequado.
                    // A lógica existente parece ser sobre um índice na escala, não um vértice da forma.
                    let randomWalkIndex = shape.currentEdgeIndex; // Reutilizar currentEdgeIndex para simplicidade
                    ...
                    randomWalkIndex = (randomWalkIndex + scaleNoteCount) % scaleNoteCount;
                    shape.currentEdgeIndex = randomWalkIndex; // Salva o novo índice

                    notesToPlay.push({ note: getNoteInScale(randomWalkIndex), vertexIndex: randomWalkIndex, isRandomWalk: true });
                    notePlayedThisTick = true;
                    shape.lastNotePlayedTime = now;
                }
                break;
        ...
        if (notePlayedThisTick && notesToPlay.length > 0) {
            ...
            // V61: notesToPlay agora deve ter apenas uma nota para ARPEGGIO, SEQUENTIAL, CHORD
            notesToPlay.forEach((noteData) => {
                ...
                // Limpa timer staccato existente para esta chave, se houver, ANTES de tocar a nova nota
                if (shape.activeMidiNotes[key]?.staccatoTimer) {
                    ...
                    // Se a nota era a mesma e estava em staccato, ela já foi desligada pelo sendMidiNoteOff acima (se !staccatoModeActive)
                    // ou o timer foi limpo. Agora vamos (re)ligá-la.
                }
                 // Se a nota já estava tocando (não staccato), o sendMidiNoteOff no início da função já a desligou.
                ...
                shape.activeMidiNotes[key] = {
                    ...
                    isChordNote: !!noteData.isChordNote, // V61
                    isRandomWalkNote: !!noteData.isRandomWalk, //V61
                    vertexIndex: vertexIndex, // V61: Adicionar vertexIndex para facilitar a limpeza e visualização
                    timestamp: now
                };

                if (staccatoModeActive) {
                    const timerId = setTimeout(() => {
                        if (shape.activeMidiNotes[key]?.playing) { // Verifica se a nota ainda deve estar tocando
    ...
    // V61: A lógica de Pitch Bend é aplicada se houver notas ativas e o pitch bend mudou.
    // Mantida a lógica da v59.
...
function sendMidiNoteOn(note, velocity, channel, shapeId = -1) {
  ...
  // const currentSimpleSynth = getSimpleSynthInstance(); // Removido - simpleSynth é global
...
function turnOffAllActiveNotesForShape(shape) { // Renomeada para manter compatibilidade, mas agora chama a nova função
...
function turnOffAllActiveNotes() {
    ...
    const origMidiEnabled = midiEnabled; // Salva o estado original do MIDI
    midiEnabled = true; // Garante que sendMidiNoteOff funcione se midiEnabled global estiver false

    shapes.forEach(shape => stopAllNotesForShape(shape, true)); // Chama a nova função centralizada
    ...
    clearAllNoteTimers(); // V59: Limpa todos os timers de nota pendentes
}
...
function resetMidiSystem() {
    ...
    turnOffAllActiveNotes(); // Isso já chama stopAllNotesForShape e clearAllNoteTimers

    const origMidiEnabled = midiEnabled;
    midiEnabled = true; // Temporariamente habilita MIDI para enviar mensagens de reset
    if (midiOutput) {
...
function setupEventListeners() {
    ...
    if (internalAudioToggleButton) {
        internalAudioToggleButton.addEventListener("click", async () => {
            try {
                if (!_internalAudioEnabledMaster) { // Se está desligado, vamos ligar
                    if (!audioCtx) {
                ...
                } else { // Se está ligado, vamos desligar
                    if (simpleSynth) {
                        simpleSynth.allNotesOff(); // Para todas as notas ao desativar
                    }
                    ...
                    // Não é necessário suspender o audioCtx aqui, apenas paramos de usá-lo.
                    // Ele pode ser resumido novamente se o usuário reativar.
...
function handleSynthControlChange(param, value) {
    if (spectatorModeActive) return;
    // const synth = getSimpleSynthInstance(); // Removido - simpleSynth é global
...
function updateModalSynthControls() {
    ...
    if (audioDelayMixSlider) audioDelayMixSlider.value = Math.acos(simpleSynth.delayDryGain.gain.value) / (0.5 * Math.PI); // Inverse of equal power
...
function updateSidebarSynthControls() {
    ...
    if (scDelayMixSlider) scDelayMixSlider.value = Math.acos(simpleSynth.delayDryGain.gain.value) / (0.5 * Math.PI); // Inverse of equal power
...
// function toggleInternalAudio() { // REMOVIDA - Lógica agora no event listener do botão
//   if (spectatorModeActive) return;
//   _internalAudioEnabledMaster = !_internalAudioEnabledMaster; // Usa a nova variável master
//   if (internalAudioToggleButton) {
...
//   }
//   // const currentSimpleSynth = getSimpleSynthInstance(); // Removido
//   // if (typeof setInternalAudioEnabledState === "function") { // Removido
//   //     setInternalAudioEnabledState(_internalAudioEnabledMaster);
//   // }
//   if (!_internalAudioEnabledMaster && simpleSynth) { // Usa simpleSynth global
//     simpleSynth.allNotesOff();
//   }
//   sendOSCMessage('/global/state/internalAudioEnabled', _internalAudioEnabledMaster ? 1 : 0);
//   updateHUD();
//   saveAllPersistentSettings();
// }
...
function updateHUD() {
  ...
  // const currentAudioCtx = getAudioContext(); // Removido - audioCtx é global
  ...
  txt += `&nbsp;&nbsp;Escala:${SCALES[currentScaleName].name} Nota:${currentNoteMode} Oper:${operationMode==='one_person'?'1P':'2P'}<br>`; // Removido Acorde de aqui
  if (currentNoteMode === 'ARPEGGIO') txt += `&nbsp;&nbsp;Arp: ${currentArpeggioStyle} BPM:${arpeggioBPM.toFixed(0)}${externalBPM!==null?'(Ext)':''}<br>`; // Idx já está na linha da forma
...
function saveAllPersistentSettings(){
  ...
  savePersistentSetting('internalAudioEnabled', _internalAudioEnabledMaster); // Salva o estado master
  // const currentSimpleSynth = getSimpleSynthInstance(); // Removido - simpleSynth é global
  if(simpleSynth) {
  ...
  console.log("Configs V55 salvas no localStorage.");
}
...
function loadAllPersistentSettings(){
  ...
  _internalAudioEnabledMaster = loadPersistentSetting('internalAudioEnabled', false); // Inicia como false por padrão
  ...
  // V55: O estado do painel synth (synthPanelHidden) é carregado e aplicado em setupEventListeners e initSynthControlsSidebar

  console.log("Configs V55 carregadas do localStorage.");
  return {
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main61.js...");
  console.log("DOM Carregado. Iniciando main61.js (v61)..."); // Log de versão atualizado
    detectPlatform();
    ...
    // Removida a lógica de 'firstGestureHandler' e a inicialização de áudio dependente dela.
    // A inicialização de audioCtx e simpleSynth agora é feita exclusivamente pelo clique
    // no botão 'internalAudioToggleButton'. As configurações de áudio são aplicadas
    // nesse momento se simpleSynth for criado.

    // Se _internalAudioEnabledMaster for true (carregado do localStorage) e audioCtx/simpleSynth não existirem,
    // o usuário ainda precisará clicar no botão para inicializá-los.
    // No entanto, se eles existirem E _internalAudioEnabledMaster for true, o som deve funcionar.
    // Se audioCtx existir mas estiver suspenso, o clique no botão também o resumirá.

    // A lógica de aplicar configurações de áudio se o contexto já estiver rodando
    // (ex: recarregamento rápido) é mantida, mas adaptada para usar audioCtx e simpleSynth globais.
    if (audioCtx && audioCtx.state === 'running') {
        console.log("AudioContext já estava rodando na inicialização da página (v60).");
        if (!simpleSynth) { // Se o synth não existe, mas o contexto sim (caso raro ou de recarregamento)
            simpleSynth = new SimpleSynth(audioCtx);
            console.log("SimpleSynth instanciado porque AudioContext já rodava (v60).");
        }
        // Aplicar configurações de áudio carregadas
        if (simpleSynth && audioSettings) { // audioSettings vem de loadAllPersistentSettings()
            ...
            updateModalSynthControls();
            updateSidebarSynthControls();
            console.log("Configurações do synth aplicadas (AudioContext já rodava) (v60).");
        }
    } else if (audioCtx && audioCtx.state === 'suspended') {
        console.log("AudioContext existe mas está suspenso na inicialização. Aguardando clique no botão de áudio. (v60)");
    } else {
        console.log("AudioContext não existe ou não está rodando na inicialização. Aguardando clique no botão de áudio. (v60)");
    }
    ...
    initMidi().then(async () => {
        ...
        // Tenta popular e inicializar câmera mesmo com erro MIDI
    ...
    if(hudElement && !loadPersistentSetting('hudHidden', false) ) { // Carrega estado do HUD
  ...
  if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas (v61).\n`;

  console.log("Iniciando loop de animação (v61) e finalizando DOMContentLoaded.");
    animationLoop();
});

function animationLoop() {
  requestAnimationFrame(animationLoop);
  // A lógica de desenho do fallback é chamada em onFrame se cameraError for true
  // e a simulação não estiver ativa. Não precisa estar aqui diretamente.
  // Apenas updateHUD pode ser útil se houver mudanças dinâmicas fora do onResults.
  // No entanto, onResults já chama updateHUD.
  // Se cameraError for true e não houver hands.send, onResults não será chamado.
  // Portanto, um updateHUD aqui para o caso de erro de câmera pode ser útil.
  if (cameraError && !gestureSimulationActive) {
      // drawFallbackAnimation(); // Já chamado em onFrame/initializeCamera
      updateHUD(); // Garante que o HUD reflita o erro da câmera
  }
}

// V61: Adicionar event listener para o seletor de modo de nota (se ainda não existir de forma adequada)
// No entanto, o `setupEventListeners` já tem um listener para `arpeggioStyleSelect`.
// A função `populateArpeggioStyleSelect` também é chamada em `DOMContentLoaded`.
// A variável `currentNoteMode` é crucial e seu controle deve ser verificado.
// O plano original mencionava NOTE_MODES e currentNoteModeIndex, que parecem ser para
// alternar entre SEQUENTIAL, ARPEGGIO, CHORD, RANDOM_WALK.
// A tarefa atual foca em refinar ARPEGGIO e CHORD, e o seletor é para ARPEGGIO STYLE.
// O `currentNoteMode` parece ser alternado por teclas ou outra UI não especificada para modificação aqui.
// Vamos garantir que a mudança de `currentArpeggioStyle` via select dispare um reset nas notas do arpejo.
// Isso já deve acontecer implicitamente pela forma como `processShapeNotes` e `stopPreviousNotesIfNeeded` funcionam,
// mas uma limpeza explícita das notas de arpejo da forma pode ser benéfica ao mudar o estilo.

// Em `setupEventListeners`, o listener para `arpeggioStyleSelect` é:
// if (arpeggioStyleSelect) arpeggioStyleSelect.addEventListener('change', (e) => {
//    if(spectatorModeActive)return;
//    currentArpeggioStyle = e.target.value;
//    saveArpeggioSettings();
//    updateHUD();
//    sendOSCMessage('/global/state/arpeggioStyle', currentArpeggioStyle);
//    // Adicionar aqui, se necessário, uma chamada para limpar notas de arpejo:
//    // shapes.forEach(shape => turnOffAllActiveNotesForShape(shape)); // Ou uma versão mais específica para arpejo.
//    // No entanto, a lógica de `processShapeNotes` e `stopPreviousNotesIfNeeded` deve lidar com isso.
// });
// A limpeza de notas ao mudar de estilo de arpejo parece ser tratada pela lógica existente,
// pois a próxima nota do arpejo será diferente ou o índice será resetado, e as notas antigas
// serão paradas por `stopPreviousNotesIfNeeded` ou pela limpeza geral em `drawShape`.

// === V52: Audio Recording Functions ===
function startAudioRecording() {
    // const currentSimpleSynth = getSimpleSynthInstance(); // Removido
    // const currentAudioCtx = getAudioContext(); // Removido
    ...
    try {
        ...
        mediaRecorder.onstart = () => {
            ...
            isAudioPaused = false;
            if (recordAudioButton) {
                recordAudioButton.innerHTML = '<span class="recording-dot">🔴</span> Parar Gravação'; // V59: Feedback visual
            ...
            updateAudioRecordingHUD(true, false, 0); // V59
            logOSC("SYSTEM", "Gravação de Áudio Iniciada", []);
            displayGlobalError("Gravação de áudio iniciada.", 3000);
        };

        mediaRecorder.onstop = () => {
            // isAudioRecording é setado para false em stopAudioRecording() que chama mediaRecorder.stop()
            // Esta flag onstop é mais para cleanup e UI final.
            isAudioPaused = false; // Garante que o estado de pausa seja resetado
            if (recordAudioButton) {
            ...
            updateAudioRecordingHUD(false, false, 0); // V59: Atualiza HUD para estado parado
            // Não exibir "Gravação parada" se foi um stop para salvar, saveRecordedAudio já informa.
            // Apenas se o stop foi manual e não resultou em chunks para salvar.
            if (audioChunks.length === 0 && !isSavingAudio) { // isSavingAudio é uma nova flag potencial
            ...
            try {
              if(simpleSynth && simpleSynth.masterGainNode && destinationNode && destinationNode.numberOfOutputs > 0) { // Checa se ainda está conectado
                simpleSynth.masterGainNode.disconnect(destinationNode);
        ...
        mediaRecorder.onpause = () => {
            if (pauseAudioButton) pauseAudioButton.innerHTML = "▶️ Retomar"; // V59: Feedback visual
            if (recordAudioButton) recordAudioButton.classList.add('paused'); // V59: Estilo para botão de gravação pausado
            isAudioPaused = true;
            updateAudioRecordingHUD(true, true, mediaRecorder.stream.currentTime); // V59
            logOSC("SYSTEM", "Gravação de Áudio Pausada (onpause)", []);
        };

        mediaRecorder.onresume = () => {
            if (pauseAudioButton) pauseAudioButton.innerHTML = "⏸️ Pausar"; // V59
            if (recordAudioButton) recordAudioButton.classList.remove('paused'); // V59
            isAudioPaused = false;
            updateAudioRecordingHUD(true, false, mediaRecorder.stream.currentTime); // V59
        ...
        mediaRecorder.onerror = (event) => {
            ...
            stopAudioRecording(true); // Passa true para indicar que é um stop por erro
        };

        mediaRecorder.start(1000); // V59: Opcional: ondataavailable a cada segundo para atualizar duração
        ...
        isAudioRecording = false; // Garante que o estado seja resetado
        updateAudioRecordingHUD(false, false, 0);
    }
}

let isStoppingDueToError = false; // Flag para evitar múltiplos logs/displays de erro

function stopAudioRecording(dueToError = false) {
    isStoppingDueToError = dueToError;
    if (mediaRecorder && (mediaRecorder.state === "recording" || mediaRecorder.state === "paused")) {
        try {
            mediaRecorder.stop(); // onstop será chamado
        } catch (e) {
            console.error("Erro ao chamar mediaRecorder.stop():", e);
            displayGlobalError("Erro ao parar gravação.", 5000);
        }
    }
    // As atualizações de UI e estado (isAudioRecording = false) são feitas principalmente em mediaRecorder.onstop
    // para garantir que ocorram após o processamento final dos dados pelo MediaRecorder.
    // No entanto, forçamos o estado aqui para consistência imediata se onstop não for chamado por algum motivo.
    isAudioRecording = false;
    isAudioPaused = false;

    // Atualiza botões caso onstop não seja chamado ou para garantir estado imediato
    if (recordAudioButton) {
        recordAudioButton.innerHTML = "⏺️ Gravar Áudio";
        recordAudioButton.classList.remove('active', 'recording', 'paused');
    }
...
let isSavingAudio = false; // V59: Flag para controlar o processo de salvamento
...
function saveRecordedAudio() {
    ...
    a.href = url;
    const fileExtension = mimeType.includes('ogg') ? 'ogg' : (mimeType.includes('mp4') ? 'mp4' : 'webm'); // V59: Melhor detecção de extensão
    ...
    updateAudioRecordingHUD(false, false, 0, true); // V59: Indica que foi salvo

    audioChunks = []; // Limpa os chunks após salvar
    if (saveAudioButton) saveAudioButton.disabled = true;
    if (recordAudioButton) recordAudioButton.classList.remove('active', 'recording', 'paused'); // Garante reset do botão de gravar

    isSavingAudio = false; // V59
}


// V59: HUD para Gravação de Áudio
...
function updateAudioRecordingHUD(isRecording, isPaused, durationSeconds = 0, isSaved = false) {
    ...
    if (!hudRecordDiv) {
        ...
        Object.assign(hudRecordDiv.style, {
            position: 'fixed',
            top: '60px', // Abaixo do sidebar handle e botão synth
            left: '50%',
            transform: 'translateX(-50%)',
            padding: '8px 15px',
            backgroundColor: 'rgba(200, 0, 0, 0.7)', // Vermelho para gravação
            color: 'white',
            zIndex: '1005', // Acima do HUD principal, abaixo de modais
            borderRadius: '5px',
    ...
    if (isRecording) {
        ...
        audioRecordingHUDTimer = setInterval(() => {
            ...
            if (isPaused) {
                ...
                hudRecordDiv.style.backgroundColor = 'rgba(255, 165, 0, 0.7)'; // Laranja para pausado
            } else {
                hudRecordDiv.textContent = `🔴 GRAVANDO (${timeStr})`;
                hudRecordDiv.style.backgroundColor = 'rgba(200, 0, 0, 0.7)'; // Vermelho
            }
        }, 1000);
         // Executa uma vez imediatamente para mostrar o estado correto
        ...
        hudRecordDiv.style.backgroundColor = 'rgba(0, 128, 0, 0.7)'; // Verde para salvo
        setTimeout(() => {
            hudRecordDiv.style.opacity = '0';
            setTimeout(() => { hudRecordDiv.style.display = 'none'; }, 300);
        }, 2700); // Esconde após ~3 segundos
    } else { // Gravação parada e não salva (ou estado inicial)
        if (hudRecordDiv.style.display === 'block') { // Se estava visível
            hudRecordDiv.style.opacity = '0';
            setTimeout(() => { hudRecordDiv.style.display = 'none'; }, 300);
        }
    }
