// ==========================================================================
// SYNTHESIZER MODULE v54 - synth54.js
// ==========================================================================
...
class SimpleSynth {
  constructor(audioContext) {
    ...
    console.log("SimpleSynth v54 inicializado com AudioContext, ADSR, DistortionNode, FilterNode, LFO, Delay e Reverb."); // v54 Update
  }
  ...
  noteOn(midiNote, velocity = 127) {
    if (this.audioCtx.state === 'suspended') {
      this.audioCtx.resume().then(() => {
        console.log("AudioContext resumed by noteOn in synth54.js"); // v54 Update
        this._playNote(midiNote, velocity);
      }).catch(e => console.error("Error resuming AudioContext in synth54.js:", e)); // v54 Update
      return;
    }
  ...
  allNotesOff() {
    console.log("Synth v54 All Notes Off (ADSR aware)"); // v54 Update
    const now = this.audioCtx.currentTime;
  ...
  if (!audioCtx) {
    try {
      ...
      if (audioCtx.state === 'suspended') {
        console.log("AudioContext (v54) está suspenso. Requer interação do usuário para iniciar."); // v54 Update
      }
      simpleSynth = new SimpleSynth(audioCtx);
      console.log("AudioContext e SimpleSynth (v54) inicializados."); // v54 Update
      return true;
    } catch (e) {
      console.error("Web Audio API não é suportada neste navegador (v54).", e); // v54 Update
      return false;
    }
  }
  return true;
...
function initAudioContextOnGesture() {
  if (!audioCtx) {
    if (!initAudioContext()) {
        console.error("Falha ao inicializar AudioContext no gesto (v54)."); // v54 Update
        return false;
    }
  }
  ...
  if (audioCtx && audioCtx.state === 'suspended') {
    audioCtx.resume().then(() => {
      console.log('AudioContext (v54) resumido com sucesso por gesto do usuário!'); // v54 Update
      if (!simpleSynth) {
          simpleSynth = new SimpleSynth(audioCtx);
          console.log("SimpleSynth (v54) instanciado após resumo do AudioContext."); // v54 Update
      }
      ...
      resumed = true;
        ...
        console.error('Erro ao resumir AudioContext (v54):', e); // v54 Update
        resumed = false;
    });
  } else if (audioCtx && audioCtx.state === 'running') {
    console.log('AudioContext (v54) já está rodando.'); // v54 Update
    resumed = true;
  } else {
    console.warn('AudioContext (v54) não está suspenso, mas também não está rodando, ou não foi inicializado.'); // v54 Update
    resumed = false;
  }
  return resumed;
}
...
function setInternalAudioEnabledState(enabled) {
    ...
    console.log(`Synth v54 _internalAudioEnabledMaster state set to: ${_internalAudioEnabledMaster}`); // v54 Update
}
...
console.log("synth54.js carregado."); // v54 Update
