...
import { getSimpleSynthInstance, getAudioContext, initAudioContextOnGesture, setInternalAudioEnabledState } from './synth51.js';
...
function sendMidiNoteOn(note, velocity, channel, shapeId = -1) {
  ...
  if (internalAudioEnabled && currentSimpleSynth && typeof currentSimpleSynth.noteOn === 'function') {
    currentSimpleSynth.noteOn(n, v); // NoteOn no synth
  }
...
function sendMidiNoteOff(note, channel, shapeId = -1) {
  ...
  if (internalAudioEnabled && currentSimpleSynth && typeof currentSimpleSynth.noteOff === 'function') {
    currentSimpleSynth.noteOff(n); // NoteOff no synth
  }
...
function toggleInternalAudio() {
  ...
  const currentSimpleSynth = getSimpleSynthInstance();
  // setInternalAudioEnabledState é importado de synth51.js
  setInternalAudioEnabledState(internalAudioEnabled);

  if (!internalAudioEnabled && currentSimpleSynth) {
    currentSimpleSynth.allNotesOff(); // Chama allNotesOff no synth
  }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const firstGestureHandler = async () => { // Tornada async
        console.log("Primeiro gesto detectado, tentando inicializar o áudio via synth51.js...");
        // initAudioContextOnGesture é importado de synth51.js
        ...
        if (audioReady) {
            const synthAfterGesture = getSimpleSynthInstance();
                ...
                console.warn("initAudioContextOnGesture() de synth51.js não retornou sucesso. O áudio pode não estar pronto.");
            }
        // A função initAudioContextOnGesture é importada, então o else não é mais necessário.
    };
