// ==========================================================================
// MIDI SHAPE MANIPULATOR v59 - main59.js
// ==========================================================================

// simpleSynth e audioCtx são gerenciados e obtidos de synth59.js (ou versão compatível)
...
// Variáveis audioCtx e simpleSynth serão obtidas de synth59.js
...
function drawShape(shape, isPulsing, pulseValue) {
  ...
  if (currentNoteMode === 'ARPEGGIO' && shape.sides > 0 && midiEnabled) {
    // V59: Chave de nota ativa simplificada para Arpeggio
...
function processShapeNotes(shape, isPulsing, pulseValue) {
    ...
    if (canPlayNonArp || canPlayArp) {
        ...
        // V59: Lógica de desligamento de notas centralizada
        stopPreviousNotesIfNeeded(shape);

        // Limpeza específica para Chord mode, que recria todas as notas
        if (currentNoteMode === 'CHORD' && canPlayNonArp) {
            stopAllNotesForShape(shape, true); // true para limpar activeMidiNotes
        }
        ...
        switch (currentNoteMode) {
            case 'SEQUENTIAL':
                if (canPlayNonArp && shape.sides > 0) {
                    // No modo SEQUENTIAL, todas as notas dos vértices são tocadas.
                    // A lógica de desligar notas anteriores já foi feita por stopPreviousNotesIfNeeded.
                    for (let i = 0; i < shape.sides; i++) {
                        notesToPlay.push({ note: getNoteInScale(i), vertexIndex: i, isSequential: true });
                    }
                    notePlayed = true;
                ...
                break;
            case 'ARPEGGIO':
                if (canPlayArp && shape.sides > 0) { // Adicionado shape.sides > 0
                    // A lógica de desligar a nota anterior do arpejo já foi feita por stopPreviousNotesIfNeeded.
                ...
                break;
            case 'CHORD':
                if (canPlayNonArp && shape.sides > 0) { // Adicionado shape.sides > 0
                    ...
                    // V59: stopAllNotesForShape já limpou activeMidiNotes para CHORD
                    notesToPlay.push({ note: getNoteInScale(edgeIdx), vertexIndex: edgeIdx, isChordNote: true, chordPart: 0 });
                    notesToPlay.push({ note: getNoteInScale(edgeIdx + 2), vertexIndex: edgeIdx, isChordNote: true, chordPart: 1 });
                    notesToPlay.push({ note: getNoteInScale(edgeIdx + 4), vertexIndex: edgeIdx, isChordNote: true, chordPart: 2 });
                    notePlayed = true; shape.lastNotePlayedTime = now;
                }
                break;
            case 'RANDOM_WALK':
                if (canPlayNonArp) { // Não depende de shape.sides para tocar
                    shape.currentEdgeIndex += Math.floor(Math.random() * 3) - 1;
                    const scaleNoteCount = SCALES[currentScaleName].notes.length * 2; // Ex: 2 oitavas da escala
                    shape.currentEdgeIndex = (shape.currentEdgeIndex + scaleNoteCount) % scaleNoteCount;
                    edgeIdx = shape.currentEdgeIndex;
                    notesToPlay.push({ note: getNoteInScale(edgeIdx), vertexIndex: edgeIdx }); // Não é especificamente sequencial ou arpejo
                    notePlayed = true; shape.lastNotePlayedTime = now;
                }
                break;
        }
        ...
        if (notePlayed && notesToPlay.length > 0) {
            ...
            notesToPlay.forEach((noteData) => {
                ...
                const vertexIndex = noteData.vertexIndex; // O índice do vértice ou base da nota
                
                // V59: Geração de chave simplificada e mais robusta
                let key;
                if (noteData.isSequential) {
                    ...
                    // Para acordes, cada parte do acorde precisa de uma chave única se vierem do mesmo vertexIndex
                    key = `shape_${shape.id}_chord_${vertexIndex}_part_${noteData.chordPart}`;
                } else { // Random Walk ou outros
                    key = `shape_${shape.id}_vtx_${vertexIndex}_note_${noteToPlay}`; // Mais específico para evitar colisões
                }

                // Limpa timer staccato existente para esta chave, se houver
                ...
                shape.activeMidiNotes[key] = {
                    note: noteToPlay,
                    playing: true,
                    staccatoTimer: null, // Será definido abaixo se staccatoModeActive
                    isSequentialNote: !!noteData.isSequential,
                    isArpeggioNote: !!noteData.isArpeggio,
                    timestamp: now // V59: Adiciona timestamp para possível garbage collection futuro
                };
                ...
                if (staccatoModeActive) {
                    const timerId = setTimeout(() => {
                        if (shape.activeMidiNotes[key]?.playing) {
                            ...
                            // V59: Não deletar a nota aqui, a limpeza geral em drawShape cuida disso
                        }
                        activeNoteTimers.delete(timerId); // V59: Remover do Set ao completar
                    }, 150);
                    shape.activeMidiNotes[key].staccatoTimer = timerId;
                    activeNoteTimers.add(timerId); // V59: Adicionar ao Set
                }
            });
    ...
    // V59: A lógica de Pitch Bend foi movida para ser aplicada apenas se houver notas ativas e se o pitch bend mudou significativamente.
    // Isso evita envios desnecessários de pitch bend.
    let activeNotesExist = false;
    let lastKnownPitchBend = 8192; // Default pitch bend
    for (const key in shape.activeMidiNotes) {
        if (shape.activeMidiNotes[key]?.playing) {
            activeNotesExist = true;
            lastKnownPitchBend = shape.activeMidiNotes[key].lastPitchBend || 8192; // Pega o último PB conhecido da primeira nota ativa
            break;
        }
    }

    if (activeNotesExist) {
        if (Math.abs(shape.currentPitchBend - lastKnownPitchBend) > 10) { // Limiar para enviar pitch bend
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main59.js...");
  console.log("DOM Carregado. Iniciando main59.js (v59)..."); // Log de versão atualizado
    detectPlatform();
    ...
    const firstGestureHandler = () => {
        console.log("Primeiro gesto detectado, tentando inicializar o áudio via synth59.js...");
        if (typeof initAudioContextOnGesture === "function") {
            const audioReady = initAudioContextOnGesture(); // Tenta iniciar/resumir o AudioContext
            if (audioReady) { // Se o contexto está pronto (ou foi resumido)
                const synthInstance = getSimpleSynthInstance(); // Agora deve retornar a instância criada ou já existente
                if (synthInstance) {
                    ...
                    Object.keys(audioSettings).forEach(key => {
                        ...
                        } else if (key === 'masterVolume' && typeof synthInstance.setMasterVolume === 'function') { // caso especial para masterVolume
                            synthInstance.setMasterVolume(audioSettings[key]);
                        }
                    });
                    updateModalSynthControls();
                    updateSidebarSynthControls();
                    console.log("Áudio inicializado/resumido por gesto e synth configurado com valores carregados (v59).");
                } else {
                    displayGlobalError("Falha ao obter instância do synth após ativar áudio.", 7000);
                    console.error("Falha ao obter instância do SimpleSynth após initAudioContextOnGesture (v59).");
                }
                updateHUD();
                ...
                console.log("Listener de primeiro gesto para áudio removido (v59).");
            } else {
                // Não necessariamente um erro fatal, o usuário pode tentar de novo.
                console.warn("initAudioContextOnGesture() de synth59.js não confirmou prontidão imediata. O áudio pode não estar pronto. Aguardando nova interação.");
                // displayGlobalError("Clique novamente para ativar o áudio.", 5000); // Opcional: informar o usuário
            }
        } else {
            displayGlobalError("Função de inicialização de áudio (initAudioContextOnGesture) não encontrada.", 10000);
            console.error("initAudioContextOnGesture não está definida globalmente (esperada de synth59.js).");
        }
    };

    document.addEventListener('click', firstGestureHandler, { once: false });
    document.addEventListener('keydown', firstGestureHandler, { once: false });
    console.log("Listeners para o primeiro gesto do usuário (para áudio) adicionados (v59).");

    // Tenta aplicar configurações de áudio se o contexto já estiver rodando (ex: recarregamento rápido)
    const currentAudioCtxState = getAudioContext()?.state;
    if (currentAudioCtxState === 'running') {
        console.log("AudioContext já estava rodando. Tentando aplicar configurações do synth (v59).");
        const synthInstance = getSimpleSynthInstance(); // Garante que o synth seja instanciado se ainda não for
        if (synthInstance) {
             Object.keys(audioSettings).forEach(key => {
                const setterName = `set${key.charAt(0).toUpperCase() + key.slice(1)}`;
                if (typeof synthInstance[setterName] === 'function') {
            ...
            document.removeEventListener('click', firstGestureHandler);
            document.removeEventListener('keydown', firstGestureHandler);
            console.log("Listeners de primeiro gesto para áudio removidos pois o áudio já está ativo (v59).");
        } else {
            console.warn("AudioContext rodando, mas falha ao obter instância do synth para aplicar configurações (v59).");
        }
    }
  ...
  if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas (v59).\n`;

  console.log("Iniciando loop de animação (v59) e finalizando DOMContentLoaded.");
    animationLoop();
});
...
function startAudioRecording() {
    ...
    try {
        ...
        mediaRecorder.onstop = () => {
            ...
            try {
              if(currentSimpleSynth && currentSimpleSynth.masterGainNode && destinationNode && destinationNode.numberOfOutputs > 0) { // Checa se ainda está conectado
                currentSimpleSynth.masterGainNode.disconnect(destinationNode);
