...
let internalAudioEnabled = true; // Estado padrão, pode ser alterado por main46.js
...
class SimpleSynth {
  ...
  noteOn(midiNote, velocity = 127) {
    if (!internalAudioEnabled || this.audioCtx.state === 'suspended') {
...
function setInternalAudioEnabledState(enabled) {
    internalAudioEnabled = !!enabled;
    if (!internalAudioEnabled && simpleSynth) {
        simpleSynth.allNotesOff();
    }
    console.log(`Synth internalAudioEnabled state set to: ${internalAudioEnabled}`);
}
