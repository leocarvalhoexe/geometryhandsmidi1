...
// These should already exist from the previous step of creating main14.js,
// but ensuring they are here and correctly placed if they were missed.
// If they are already present, this block will effectively do nothing.
let handsCanvas;
let handsCtx;
...
function drawLandmarksToContext(targetCtx, landmarks, canvasWidth, canvasHeight) {
  if (!targetCtx || !canvasWidth || !canvasHeight) return; // Adapted guard clause
  const connections = [[0,1],[1,2],[2,3],[3,4],[0,5],[5,6],[6,7],[7,8],[5,9],[9,10],[10,11],[11,12],[9,13],[13,14],[14,15],[15,16],[13,17],[17,18],[18,19],[19,20],[0,17]];
  targetCtx.strokeStyle='lime'; // Changed ctx to targetCtx
  targetCtx.lineWidth=2; // Changed ctx to targetCtx
  for(const [startIdx, endIdx] of connections){
    const x1 = canvasWidth - (landmarks[startIdx].x * canvasWidth); // Adapted cW
    const y1 = landmarks[startIdx].y * canvasHeight; // Adapted cH
    const x2 = canvasWidth - (landmarks[endIdx].x * canvasWidth); // Adapted cW
    const y2 = landmarks[endIdx].y * canvasHeight; // Adapted cH
    targetCtx.beginPath(); // Changed ctx to targetCtx
    targetCtx.moveTo(x1,y1); // Changed ctx to targetCtx
    targetCtx.lineTo(x2,y2); // Changed ctx to targetCtx
    targetCtx.stroke(); // Changed ctx to targetCtx
  }
}
...
function onResults(results) {
  ...
  // Normal mode (single shape) hand controls
    ...
    if ((!mouseControlsActive || !isMouseDown) && results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        // if (leftHand) drawLandmarks(leftHand); // Old direct drawing to main canvas
        // if (rightHand) drawLandmarks(rightHand); // Old direct drawing to main canvas
  ...
  // Clear hands canvas and draw landmarks to it
  if (handsCtx && handsCanvas) { // Ensure they exist
    handsCtx.clearRect(0, 0, handsCanvas.width, handsCanvas.height);
    if (results.multiHandLandmarks) {
        for (const landmarks of results.multiHandLandmarks) {
            drawLandmarksToContext(handsCtx, landmarks, handsCanvas.width, handsCanvas.height);
        }
    }
  }
  ...
  // Composite handsCanvas onto the main canvas
  if (ctx && handsCanvas) {
    ctx.drawImage(handsCanvas, 0, 0);
  }
  ...
  if (outputPopupWindow && !outputPopupWindow.closed && popupCanvasCtx) {
    try {
      ...
      if (popupCanvas) {
        ...
        // Replace drawImage with drawShapeForPopup
        if (selectedShape) {
            drawShapeForPopup(
                popupCanvasCtx,
                popupCanvas.width,
                popupCanvas.height,
                selectedShape,
                selectedShape.radius, // Base radius for popup
                false, // isPulsingActive for popup
                0,     // pulseCycleValue for popup
                0,     // currentDemoAngle for popup
                false, // isDemoDistortingFlag for popup
                true,  // useGlobalRightHandLandmarks (to reflect main window's liquify)
                cW,    // globalCanvasWidth (main canvas width for liquify scaling)
                cH     // globalCanvasHeight (main canvas height for liquify scaling)
            );
        }
      }
    } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvasElement) {
        ...
        // Initialize offscreen canvas for hands
        handsCanvas = document.createElement('canvas');
        handsCtx = handsCanvas.getContext('2d');
        if (canvasElement) { // Ensure main canvas exists
...
function resizeCanvas() {
  if (canvasElement) {
    ...
    if (handsCanvas) { // Ensure handsCanvas has been initialized
        handsCanvas.width = canvasElement.width;
        handsCanvas.height = canvasElement.height;
    }
...
// New function for popup
function drawShapeForPopup(targetCtx, targetWidth, targetHeight, shape, currentPulsedRadius, isPulsingActive, pulseCycleValue, currentDemoAngle, isDemoDistortingFlag, useGlobalRightHandLandmarks, globalCanvasWidth, globalCanvasHeight) {
  if (!targetCtx || !targetWidth || !targetHeight) return; // Adapted guard clause

  targetCtx.beginPath(); // Changed from ctx
  const currentDisplaySides = Math.round(Math.max(MIN_SIDES, Math.min(MAX_SIDES, shape.sides)));
  const shapeCenterX = shape.x; // Assuming shape.x, shape.y are relative to the canvas it's drawn on
  const shapeCenterY = shape.y;
  ...
  for (let i = 0; i < currentDisplaySides; i++) {
    ...
    // Demo mode rotation is a global effect, might need to be re-evaluated if it should apply to popup
    if (demoModeActive && currentDemoAngle && shape.isSelected) {
        let rotatedX = vertexXoriginal * Math.cos(currentDemoAngle) - vertexYoriginal * Math.sin(currentDemoAngle);
        let rotatedY = vertexXoriginal * Math.sin(currentDemoAngle) + vertexYoriginal * Math.cos(currentDemoAngle);
        vertexXoriginal = rotatedX;
    ...
    // Adjusted Liquify Logic
    if (useGlobalRightHandLandmarks && rightHandLandmarks && !demoModeActive && shape.isSelected) {
        if (mouseControlsActive && !isMouseDown) applyManualLiquify = true; // mouseControlsActive and isMouseDown are global states
        else if (!mouseControlsActive) applyManualLiquify = true;
    }
    ...
    if (applyManualLiquify) {
        ...
        for (const landmarkIndex of fingerTipIndices) {
            const fingerTip = rightHandLandmarks[landmarkIndex]; // Global landmarks
            // Use globalCanvasWidth and globalCanvasHeight for landmark coordinate calculation
            const fingerX = globalCanvasWidth - (fingerTip.x * globalCanvasWidth);
            const fingerY = fingerTip.y * globalCanvasHeight;
    ...
    // Demo distortion is a global effect, might need re-evaluation for popup
     if (demoModeActive && isDemoDistortingFlag && shape.isSelected) {
        totalDistortionX += (Math.random() - 0.5) * 30;
        totalDistortionY += (Math.random() - 0.5) * 30;
    }
    ...
    // MIDI LOGIC IS COMPLETELY REMOVED FOR POPUP

    if (i === 0) targetCtx.moveTo(finalX, finalY); // Changed from ctx
    else targetCtx.lineTo(finalX, finalY); // Changed from ctx
  }

  // MIDI-related activeNotes cleanup IS COMPLETELY REMOVED FOR POPUP

  targetCtx.closePath(); // Changed from ctx
  targetCtx.strokeStyle = shape.isSelected ? 'yellow' : shape.color;
  targetCtx.lineWidth = shape.isSelected ? 6 : 4;
  targetCtx.stroke(); // Changed from ctx
}
