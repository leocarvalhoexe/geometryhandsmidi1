// ==========================================================================
// MIDI SHAPE MANIPULATOR v57 - main57.js
// ==========================================================================

// simpleSynth e audioCtx são gerenciados e obtidos de synth57.js (ou versão compatível)
...
class Shape {
  constructor(id, midiChannel) {
    ...
    // v56 & v57 - Para gestos avançados e novas funcionalidades
    this.previousFrameRadius = this.radius; // Usado em v56 e v57 (expressividade por movimento)
    this.liquifyIntensityPeaked = false; // v56
    this.nextChordShouldBeInverted = false; // v56
    this.currentInversionLevel = 0; // v56
    this.fxBoostFactor = 1.0; // v56
    this.transpositionGestureState = { raised: false, lowered: false, neutral: true }; // v56
    this.lastResizeTimeForFx = performance.now(); // v56
    this.lastSignificantChangeTime = performance.now(); // v56 & v57 (modo PAUSE automático)

    // v57 - Novas propriedades da Shape
    this.noteQueue = []; // Para quantização rítmica
    this.isVisuallyHighlighted = false; // Para feedback visual ao acionar notas
    this.lastRadiusForVelocity = this.radius; // Para cálculo de velocity
    this.lastUpdateTime = performance.now(); // Para deltaTime no cálculo de velocity
  }
}

const shapes = [new Shape(0, 0), new Shape(1, 1)];

// v56 & v57 Constantes e Variáveis Globais
let globalKeyTransposeSemits = 0; // v56
const MAX_GLOBAL_TRANSPOSE = 6; // v56
const TRANSPOSE_GESTURE_THRESHOLD_Y_FACTOR = 0.25; // v56
const FX_BOOST_RADIUS_CHANGE_SPEED_THRESHOLD = 7.0; // v56
const FX_BOOST_MULTIPLIER = 1.5; // v56
const LIQUIFY_INTENSITY_THRESHOLD_FOR_INVERSION = 100; // v56

// v57 - Constantes para novas funcionalidades
const INDICATOR_PROXIMITY_THRESHOLD_FACTOR = 0.15; // 15% do raio da forma para detectar proximidade do dedo
const DISTORTION_THRESHOLD_FOR_HARMONIC_CHANGE = 100; // modWheel > 100 para acionar acordes ou alterar progressão
const VISUAL_HIGHLIGHT_DURATION_MS = 150; // Duração do realce visual

// v56 & v57 - Constantes para Pausas Intencionais Dinâmicas
const THRESHOLD_SIDES_FOR_PAUSE = 4;
const THRESHOLD_RADIUS_FOR_PAUSE = 40;
const IDLE_TIMEOUT_FOR_PAUSE_MS = 3000;
const MIN_MUSICAL_PAUSE_INTERVAL_MS = 200; // v57: Mínimo para que a pausa seja musicalmente significativa
...
// === CADÊNCIA HARMÔNICA v56 ===
const HARMONIC_PROGRESSIONS = [
  ["C", "Am", "F", "G"], // I-vi-IV-V em C
  ["Dm", "G", "C", "Am"], // ii-V-I-vi em C
  ["C", "F", "G", "C"],  // I-IV-V-I em C
  ["Am", "G", "C", "E"],  // i-VII-III-V em Am (relativa menor de C)
  ["Cm", "Fm", "G", "Cm"] // i-iv-V-i em Cm
  // Futuro: Adicionar mais progressões e talvez agrupá-las por tonalidade/modo
];
let currentHarmonicProgression = HARMONIC_PROGRESSIONS[0];
let currentChordInProgressionIndex = 0;
let masterBeatCounter = 0;
const BEATS_PER_CHORD = 4; // Cada acorde dura 4 batidas (1 compasso de 4/4)
let lastBeatTime = 0; // Para controlar o masterBeatCounter
let activeCadenceChordNotes = {}; // Para rastrear notas do acorde da cadência (objeto: {note: midiValue, channel: X, playing: true})
const CADENCE_MIDI_CHANNEL = 2; // Canal MIDI dedicado para os acordes da cadência (0-15)
const CADENCE_CHORD_VELOCITY = 45; // Volume para os acordes da cadência
const CADENCE_BASE_OCTAVE = 3; // Oitava base para os acordes da cadência (C3, etc.)
// === FIM CADÊNCIA HARMÔNICA v56 ===

let osc;
let oscStatus = "OSC Desconectado";

// Variáveis audioCtx e simpleSynth serão obtidas de synth48.js
...
let shapePresets = {};
const APP_SETTINGS_KEY = 'midiShapeManipulatorV56Settings';
...
const NOTE_MODES = ['SEQUENTIAL', 'ARPEGGIO', 'CHORD', 'RANDOM_WALK', 'PAUSE']; // Adicionado 'PAUSE'
let currentNoteMode = 'SEQUENTIAL'; // Modo inicial padrão
let currentNoteModeIndex = 0; // Para ciclo manual, se mantido

// === ALTERNÂNCIA AUTOMÁTICA DE MODO DE NOTA v56 ===
const AUTO_NOTE_MODES = ['CHORD', 'ARPEGGIO', 'SEQUENTIAL', 'PAUSE'];
let currentAutoNoteModeIndex = 0;
const NOTE_MODE_CHANGE_INTERVAL_MEASURES = 1; // Mudar modo de nota a cada X compassos (1 compasso = BEATS_PER_CHORD batidas)
let measuresCounterForNoteMode = 0;
let automaticNoteModeChangeEnabled = true; // Flag para ativar/desativar esta funcionalidade
// === FIM ALTERNÂNCIA AUTOMÁTICA v56 ===
...
function drawShape(shape, isPulsing, pulseValue) {
  ...
  ctx.closePath();
  ctx.strokeStyle = shape.id === 0 ? '#00FFFF' : '#FF00FF';
  ctx.lineWidth = shape.isVisuallyHighlighted ? 6 : 3; // Feedback visual: aumenta a largura da borda
  if (shape.isVisuallyHighlighted && shape.id === 0) ctx.strokeStyle = '#FFFFFF'; // Cor branca para destaque na forma 0
  if (shape.isVisuallyHighlighted && shape.id === 1) ctx.strokeStyle = '#F0F0F0'; // Cor cinza claro para destaque na forma 1
  ctx.stroke();

  // Feedback visual: realce do centro (opcional, pode ser combinado ou alternativo ao da borda)
  if (shape.isVisuallyHighlighted) {
    ctx.beginPath();
    ctx.arc(cx, cy, r * 0.2, 0, Math.PI * 2); // Realce no centro
    ctx.fillStyle = shape.id === 0 ? 'rgba(255, 255, 255, 0.5)' : 'rgba(200, 200, 255, 0.5)';
    ctx.fill();
  }
...
function getNoteInScale(index, baseOctaveOffset = 0) {
  ...
  let finalNote = scale.baseMidiNote + scaleNotes[noteIdx] + (octave * 12);
  finalNote += globalKeyTransposeSemits; // Aplicar transposição global v56
  return Math.max(0, Math.min(127, finalNote));
}

// === FUNÇÕES DA CADÊNCIA HARMÔNICA v56 ===
const CHORD_QUALITIES = {
    // Mapeia qualidade do acorde para intervalos em semitons a partir da raiz
    // Raiz (0), Terça, Quinta, Sétima (opcional), Nona (opcional)
    "M": [0, 4, 7], // Maior
    "m": [0, 3, 7], // Menor
    "dim": [0, 3, 6], // Diminuto
    "aug": [0, 4, 8], // Aumentado
    "7": [0, 4, 7, 10], // Dominante 7ª
    "M7": [0, 4, 7, 11], // Maior 7ª
    "m7": [0, 3, 7, 10], // Menor 7ª
    "dim7": [0, 3, 6, 9], // Diminuto 7ª (sétima diminuta)
    "m7b5": [0, 3, 6, 10], // Meio-diminuto (menor 7ª com 5ª diminuta)
};

const NOTE_MIDI_OFFSETS = {
    "C": 0, "C#": 1, "Db": 1, "D": 2, "D#": 3, "Eb": 3, "E": 4, "Fb": 4,
    "F": 5, "F#": 6, "Gb": 6, "G": 7, "G#": 8, "Ab": 8, "A": 9,
    "A#": 10, "Bb": 10, "B": 11, "Cb": 11
};

function getChordNotes(chordName, baseOctave = 3, targetNumNotes = 3, inversionLevel = 0) {
    let rootNoteName = chordName.match(/^[A-Ga-g][#b]?/)[0];
    let qualitySymbol = chordName.substring(rootNoteName.length);
    if (qualitySymbol === "") qualitySymbol = "M"; // Default para Maior se não especificado

    const intervals = CHORD_QUALITIES[qualitySymbol] || CHORD_QUALITIES["M"]; // Fallback para Maior
    const rootMidiOffset = NOTE_MIDI_OFFSETS[rootNoteName];
    if (rootMidiOffset === undefined) {
        console.warn(`Nome de nota raiz desconhecido em getChordNotes: ${rootNoteName}`);
        return [];
    }

    const baseMidiNote = (12 * (baseOctave + 1)) + rootMidiOffset; // C0 = MIDI 12, C1 = 24,... C4 = 60

    let chordMidiNotes = intervals.map(interval => baseMidiNote + interval + globalKeyTransposeSemits); // Aplicar transposição global v56

    // Aplicar inversão
    // Nota: esta lógica de inversão é básica e pode precisar de refinamento para contextos musicais complexos.
    // Garante que a inversão não crie notas fora do alcance MIDI ou notas muito distantes.
    if (inversionLevel > 0 && chordMidiNotes.length > 1) {
        for (let i = 0; i < inversionLevel; i++) {
            if (chordMidiNotes.length > 0) {
                const firstNote = chordMidiNotes.shift();
                if (firstNote + 12 <= 127) { // Evitar notas MIDI > 127 (já transpostas)
                    chordMidiNotes.push(firstNote + 12);
                } else {
                    chordMidiNotes.unshift(firstNote); // Devolve se não puder subir oitava
                    break; // Para de inverter se atingir o limite
                }
            }
        }
    }

    // Ajustar para targetNumNotes (simplificado: pega as primeiras N notas)
    // Pode ser expandido para voicings mais inteligentes.
    chordMidiNotes = chordMidiNotes.slice(0, targetNumNotes);

    return chordMidiNotes.map(note => Math.max(0, Math.min(127, note))); // Garante que está no range MIDI (após transposição e inversão)
}

function playCadenceChord(chordNotes) {
    // Desligar notas do acorde anterior da cadência
    Object.values(activeCadenceChordNotes).forEach(noteInfo => {
        if (noteInfo.playing) {
            sendMidiNoteOff(noteInfo.note, noteInfo.channel, -2); // -2 para indicar que é da cadência
            if (noteInfo.staccatoTimer) clearTimeout(noteInfo.staccatoTimer);
        }
    });
    activeCadenceChordNotes = {};

    // Tocar novas notas do acorde
    chordNotes.forEach((note, index) => {
        const key = `cadence_${note}_${index}`;
        sendMidiNoteOn(note, CADENCE_CHORD_VELOCITY, CADENCE_MIDI_CHANNEL, -2);
        activeCadenceChordNotes[key] = {
            note: note,
            channel: CADENCE_MIDI_CHANNEL,
            playing: true,
            staccatoTimer: null // Cadência geralmente não é staccato
        };
        // Log para debug
        logDebug(`Cadence Note ON: ${getNoteName(note)} (MIDI: ${note}) Ch: ${CADENCE_MIDI_CHANNEL}`);
    });
    const currentChordName = currentHarmonicProgression[currentChordInProgressionIndex];
    sendOSCMessage('/cadence/chord', currentChordName, chordNotes);
    updateHUD(); // Para mostrar o acorde atual no HUD
}

function advanceHarmonicProgression() {
    currentChordInProgressionIndex = (currentChordInProgressionIndex + 1) % currentHarmonicProgression.length;

    // Aleatoriamente (ou com alguma lógica) pode-se mudar para uma nova progressão
    if (currentChordInProgressionIndex === 0 && Math.random() < 0.25) { // 25% de chance de mudar de progressão ao completar uma
        const newProgressionIndex = Math.floor(Math.random() * HARMONIC_PROGRESSIONS.length);
        currentHarmonicProgression = HARMONIC_PROGRESSIONS[newProgressionIndex];
        logDebug("Mudou para nova progressão harmônica:", currentHarmonicProgression);
    }

    const chordName = currentHarmonicProgression[currentChordInProgressionIndex];
    const notes = getChordNotes(chordName, CADENCE_BASE_OCTAVE);
    if (notes.length > 0) {
        playCadenceChord(notes);
    }
    logDebug(`Avançou para o acorde: ${chordName} (Índice: ${currentChordInProgressionIndex}) Notas: ${notes}`);
}

function processHarmonicCadence() {
    if (spectatorModeActive) return; // Não processar cadência em modo espectador

    const now = performance.now();
    if (lastBeatTime === 0) lastBeatTime = now; // Inicializa na primeira chamada

    if (now - lastBeatTime >= noteInterval) { // noteInterval define a batida
        masterBeatCounter++;
        lastBeatTime = now; // Atualiza para o tempo desta batida

        // A cada N batidas (definidas por BEATS_PER_CHORD), avançar o acorde
        if (masterBeatCounter % BEATS_PER_CHORD === 0) {
            advanceHarmonicProgression();
        }
    }
}

function updateMusicalStructureAndTiming() {
    if (spectatorModeActive && !isRecordingOSC && !isPlayingOSCLoop) return;

    const now = performance.now();
    if (lastBeatTime === 0) lastBeatTime = now;

    if (now - lastBeatTime >= noteInterval) { // noteInterval define a batida (pulso)
        masterBeatCounter++;
        lastBeatTime = now;
        logDebug(`Master Beat: ${masterBeatCounter}`);

        // Tocar notas da fila que estão agendadas para esta batida
        shapes.forEach(shape => {
            const notesToPlayNow = [];
            const remainingNotes = [];
            shape.noteQueue.forEach(queuedNote => {
                if (queuedNote.playTime === masterBeatCounter) {
                    notesToPlayNow.push(queuedNote);
                } else if (queuedNote.playTime > masterBeatCounter) {
                    remainingNotes.push(queuedNote);
                } // Notas com playTime < masterBeatCounter são descartadas (atrasadas demais)
            });
            shape.noteQueue = remainingNotes;

            if (notesToPlayNow.length > 0) {
                logDebug(`Forma ${shape.id}: Tocando ${notesToPlayNow.length} notas da fila na batida ${masterBeatCounter}`);
                // Antes de tocar, desligar notas ativas do mesmo "tipo" ou "contexto" para evitar sobreposição indesejada
                // (Ex: se for uma nova nota sequencial, desligar a anterior sequencial)
                // Esta lógica pode ser complexa e depende de como as chaves (keyContext) são gerenciadas.
                // Por ora, uma simplificação: se estamos tocando um acorde, desligamos todas as notas anteriores da forma.
                if (notesToPlayNow.some(n => n.type === 'chord')) {
                    turnOffAllActiveNotesForShape(shape, false); // false para não limpar a nota recém adicionada à activeMidiNotes
                }

                notesToPlayNow.forEach(qn => {
                    sendMidiNoteOn(qn.note, qn.velocity, qn.channel, qn.entityId);

                    // Adicionar à lista de notas ativas para gerenciamento de noteOff e pitch bend
                    if (shape.activeMidiNotes[qn.keyContext]?.staccatoTimer) clearTimeout(shape.activeMidiNotes[qn.keyContext].staccatoTimer);
                    shape.activeMidiNotes[qn.keyContext] = {
                        note: qn.note,
                        playing: true,
                        lastPitchBend: shape.currentPitchBend, // Assumir pitch bend atual ao tocar
                        isArpeggioNote: qn.type === 'arpeggio',
                        isSequentialNote: qn.type === 'sequential'
                        // Outros metadados relevantes podem ser adicionados aqui
                    };

                    if (staccatoModeActive) { // Staccato ainda pode ser útil para notas individuais
                        shape.activeMidiNotes[qn.keyContext].staccatoTimer = setTimeout(() => {
                            if (shape.activeMidiNotes[qn.keyContext]?.playing) {
                                sendMidiNoteOff(qn.note, qn.channel, qn.entityId);
                                if (shape.activeMidiNotes[qn.keyContext]) shape.activeMidiNotes[qn.keyContext].playing = false;
                                // Não deletar imediatamente, pode ser útil para referências futuras ou re-trigger
                            }
                        }, 150); // Duração do staccato
                    }
                });
            }
        });

        // Avançar cadência harmônica e modo de nota no tempo certo
        if (masterBeatCounter % BEATS_PER_CHORD === 0) {
            if (!spectatorModeActive) {
                // v57 - Se houver distorção intensa, pode haver uma chance de alterar a progressão harmônica
                if (shapes.some(s => s.modWheelValue > DISTORTION_THRESHOLD_FOR_HARMONIC_CHANGE) && Math.random() < 0.3) { // 30% de chance
                    const oldProgression = currentHarmonicProgression;
                    let newProgressionIndex = Math.floor(Math.random() * HARMONIC_PROGRESSIONS.length);
                    // Garantir que seja uma nova progressão, se possível
                    if (HARMONIC_PROGRESSIONS.length > 1) {
                        while (HARMONIC_PROGRESSIONS[newProgressionIndex] === oldProgression) {
                            newProgressionIndex = Math.floor(Math.random() * HARMONIC_PROGRESSIONS.length);
                        }
                    }
                    currentHarmonicProgression = HARMONIC_PROGRESSIONS[newProgressionIndex];
                    currentChordInProgressionIndex = 0; // Resetar para o início da nova progressão
                    logDebug(`Progressão harmônica alterada por distorção intensa para:`, currentHarmonicProgression);
                    sendOSCMessage('/cadence/progressionChanged', newProgressionIndex + 1, currentHarmonicProgression);
                }
                advanceHarmonicProgression();
            }

            if (automaticNoteModeChangeEnabled && !spectatorModeActive) {
                measuresCounterForNoteMode++;
                if (measuresCounterForNoteMode >= NOTE_MODE_CHANGE_INTERVAL_MEASURES) {
                    currentAutoNoteModeIndex = (currentAutoNoteModeIndex + 1) % AUTO_NOTE_MODES.length;
                    currentNoteMode = AUTO_NOTE_MODES[currentAutoNoteModeIndex];
                    measuresCounterForNoteMode = 0;
                    logDebug("Modo de Nota Automático alterado para: " + currentNoteMode);
                    sendOSCMessage('/global/state/noteMode', currentNoteMode);
                    updateHUD();
                }
            }
        }
...
// === FIM FUNÇÕES DA CADÊNCIA HARMÔNICA v56 (e lógica de quantização v57) ===

function processShapeNotes(shape, isPulsing, pulseValue) {
    // v57 - Atualizar deltaTime e lastRadiusForVelocity
    const now = performance.now(); // 'now' já definido em updateMusicalStructureAndTiming
    const deltaTime = (now - shape.lastUpdateTime) / 1000.0; // DeltaTime em segundos
    shape.lastUpdateTime = now;

    // v57 - Calcular velocity com base na mudança de raio
    let radiusVelocity = 0;
    if (deltaTime > 0) {
        const radiusDifference = Math.abs(shape.radius - shape.lastRadiusForVelocity);
        radiusVelocity = Math.min(127, Math.round(radiusDifference / deltaTime * 0.5)); // Ajuste o multiplicador (0.5) conforme necessário
    }
    shape.lastRadiusForVelocity = shape.radius;


    // v57 - Lógica de PAUSE AUTOMÁTICO APRIMORADA
    // Se o modo PAUSE foi definido pela alternância automática OU dinamicamente, respeitar.
    let isPausedThisFrame = (currentNoteMode === 'PAUSE' && automaticNoteModeChangeEnabled);

    if (!isPausedThisFrame) { // Só verifica pausa dinâmica se não estiver em pausa automática
        let isDynamicallyPaused = false;
        if (shape.sides < THRESHOLD_SIDES_FOR_PAUSE && shape.sides !== 100) {
            isDynamicallyPaused = true;
            logDebug(`Forma ${shape.id} pausada dinamicamente: poucos lados (${shape.sides})`);
        }
        if (shape.radius < THRESHOLD_RADIUS_FOR_PAUSE) {
            isDynamicallyPaused = true;
            logDebug(`Forma ${shape.id} pausada dinamicamente: raio pequeno (${shape.radius.toFixed(0)})`);
        }
        if (now - shape.lastSignificantChangeTime > IDLE_TIMEOUT_FOR_PAUSE_MS) {
            isDynamicallyPaused = true;
            logDebug(`Forma ${shape.id} pausada dinamicamente: inativa por ${IDLE_TIMEOUT_FOR_PAUSE_MS}ms`);
        }
        if (isDynamicallyPaused) isPausedThisFrame = true;
    }

    if (isPausedThisFrame) {
        // A pausa deve respeitar o tempo musical.
        // Se uma nota estava na fila, ela não será tocada. A fila é limpa.
        if (shape.noteQueue.length > 0) {
            logDebug(`Forma ${shape.id} em PAUSA, limpando fila de ${shape.noteQueue.length} notas.`);
            shape.noteQueue = [];
        }
        // Desligar notas ativas se já passou um tempo musicalmente relevante desde a última nota.
        // Isso evita que a pausa desligue notas abruptamente no meio de uma batida.
        if (now - shape.lastNotePlayedTime > MIN_MUSICAL_PAUSE_INTERVAL_MS) {
             turnOffAllActiveNotesForShape(shape);
        }
        // Atualizar lastNotePlayedTime para manter o ritmo da pausa, mesmo que não toque nada.
        // Isso garante que quando sair da pausa, não haverá uma rajada imediata de notas.
        if (now - shape.lastNotePlayedTime >= noteInterval) {
             shape.lastNotePlayedTime = now;
        }
        return; // Interrompe a geração/enfileiramento de novas notas
    }
    // FIM LÓGICA DE PAUSA APRIMORADA v57

    if (spectatorModeActive || !midiEnabled || shape.sides <= 0) return;

    // v57 - Detecção de proximidade do dedo indicador com o centro da forma
    // Usaremos a mão direita (liquify hand) para este gesto por enquanto.
    let indicatorNearCenter = false;
    if (shape.rightHandLandmarks) {
        const indicatorTip = shape.rightHandLandmarks[8]; // Ponta do dedo indicador
        const indicatorCanvasX = canvasElement.width - (indicatorTip.x * canvasElement.width);
        const indicatorCanvasY = indicatorTip.y * canvasElement.height;
        const distToCenter = distance(indicatorCanvasX, indicatorCanvasY, shape.centerX, shape.centerY);
        if (distToCenter < shape.radius * INDICATOR_PROXIMITY_THRESHOLD_FACTOR) {
            indicatorNearCenter = true;
            logDebug(`Forma ${shape.id}: Dedo indicador próximo ao centro!`);
            // Aqui, podemos definir uma nota específica ou um comportamento.
            // Por exemplo, tocar a tônica da escala atual ou um som especial.
            // Esta lógica será adicionada no enfileiramento da nota.
        }
    }

    // v57 - Acionamento por distorção intensa
    let intenseDistortionActive = shape.modWheelValue > DISTORTION_THRESHOLD_FOR_HARMONIC_CHANGE;
    if (intenseDistortionActive) {
        logDebug(`Forma ${shape.id}: Distorção intensa detectada (modWheel: ${shape.modWheelValue})`);
        // A lógica para acordes com inversão ou alteração da progressão será tratada no CHORD mode
        // e na função `advanceHarmonicProgression` ou similar.
        // Por agora, apenas sinalizamos que a condição foi atendida.
        // Se `shape.nextChordShouldBeInverted` já é usado para inversões, podemos reusá-lo
        // ou criar uma nova flag específica para este tipo de acionamento.
        // Para simplificar, vamos assumir que se `intenseDistortionActive` e modo CHORD,
        // `getChordNotes` poderá usar uma inversão maior ou uma variação do acorde.
    }
    ...
    if (canPlayNonArp || canPlayArp) {
        let notesToProcess = []; // Renomeado de notesToPlay para indicar que irão para a fila
        let edgeIdx = shape.currentEdgeIndex;
        let noteLogicProcessed = false; // Similar ao 'notePlayed' anterior
        ...
        if (currentNoteMode !== 'CHORD' && currentNoteMode !== 'ARPEGGIO') {
            ...
            if (shape.activeMidiNotes[oldKey]?.playing && !staccatoModeActive) {
                // Não desligar imediatamente, deixar a quantização e noteOff cuidarem disso.
        ...
        switch (currentNoteMode) {
            case 'SEQUENTIAL':
                if (canPlayNonArp && shape.sides > 0) {
                    for (let i = 0; i < shape.sides; i++) {
                        notesToProcess.push({ note: getNoteInScale(i), velocity: radiusVelocity, type: 'sequential', vertexIndex: i });
                    }
                    noteLogicProcessed = true;
                    shape.lastNotePlayedTime = now; // Marcar tempo de processamento da lógica da nota
                }
                break;
            case 'ARPEGGIO':
                if (canPlayArp) {
                    // Lógica de desligar notas de arpejo anteriores também pode ser removida ou ajustada com a fila
                    // Object.keys(shape.activeMidiNotes).forEach(k => { if (k.startsWith(`arp_${shape.id}_`) && ... )});
                    ...
                    if (shape.sides > 0) notesToProcess.push({ note: getNoteInScale(edgeIdx), velocity: radiusVelocity, type: 'arpeggio' });
                    noteLogicProcessed = true;
            ...
            case 'CHORD':
                if (canPlayNonArp) {
                    shape.currentEdgeIndex = (shape.currentEdgeIndex + shape.rotationDirection) % shape.sides;
                    if (shape.currentEdgeIndex < 0) shape.currentEdgeIndex = shape.sides -1;

                    edgeIdx = shape.currentEdgeIndex;
                    const currentCadenceChordName = currentHarmonicProgression[currentChordInProgressionIndex];
                    if (currentCadenceChordName) {
                        let numNotesInChord = 3;
                        if (shape.sides >= 4 && shape.sides < 7) numNotesInChord = 4;
                        else if (shape.sides >= 7) numNotesInChord = 5;

                        let baseInversionLevel = shape.currentEdgeIndex % numNotesInChord;
                        let finalInversionLevel = baseInversionLevel;

                        // v57 - Acionamento por distorção intensa para CHORD
                        if (intenseDistortionActive) {
                            // Exemplo: aumentar nível de inversão ou usar um voicing mais complexo/dissonante
                            finalInversionLevel = (baseInversionLevel + 1) % numNotesInChord; // Aumenta a inversão
                            logDebug(`Forma ${shape.id} CHORD: Distorção intensa -> Inversão alterada para ${finalInversionLevel}`);
                            // Poderia também alterar a progressão harmônica global aqui, mas requer mais cuidado
                            // if (Math.random() < 0.5) { /* chance de mudar progressão */ }
                        } else if (shape.nextChordShouldBeInverted) { // v56 liquify gesture
                            shape.currentInversionLevel = (shape.currentInversionLevel + 1) % numNotesInChord;
                            finalInversionLevel = shape.currentInversionLevel;
                            shape.nextChordShouldBeInverted = false;
                        }

                        const chordNotes = getChordNotes(currentCadenceChordName, CADENCE_BASE_OCTAVE + 1, numNotesInChord, finalInversionLevel);
                        if (chordNotes.length > 0) {
                            chordNotes.forEach(n => notesToProcess.push({ note: n, velocity: radiusVelocity, type: 'chord' }));
                            noteLogicProcessed = true;
                        }
                    }
                    if(noteLogicProcessed) shape.lastNotePlayedTime = now;
                }
                break;
            // PAUSE já tratado no início da função
            case 'RANDOM_WALK':
                if (canPlayNonArp) {
                    ...
                    edgeIdx = shape.currentEdgeIndex;
                    notesToProcess.push({ note: getNoteInScale(edgeIdx), velocity: radiusVelocity, type: 'random_walk' });
                    noteLogicProcessed = true;
                    shape.lastNotePlayedTime = now;
                }
                break;
        ...
        // v57 - Adicionar nota especial se dedo indicador estiver próximo ao centro
        if (indicatorNearCenter && notesToProcess.length === 0 && canPlayNonArp) { // Só adiciona se não houver outras notas e puder tocar
            const specialNote = getNoteInScale(0, 1); // Ex: Tônica da escala, uma oitava acima
            notesToProcess.push({ note: specialNote, velocity: Math.max(80, radiusVelocity), type: 'indicator_special' });
            logDebug(`Forma ${shape.id}: Enfileirando nota especial por proximidade do indicador: ${getNoteName(specialNote)}`);
            if (!noteLogicProcessed) shape.lastNotePlayedTime = now; // Marcar tempo se esta for a única "lógica" de nota
            noteLogicProcessed = true;
        }


        // Adicionar notas à fila de quantização
        if (noteLogicProcessed && notesToProcess.length > 0) {
            notesToProcess.forEach(noteInfo => {
                // Aplicar velocity calculada (pode ser a global da forma ou específica do gesto)
                // A velocity padrão é `radiusVelocity`, mas pode ser sobrescrita por gestos específicos
                let finalVelocity = noteInfo.velocity; // Usa a velocity já definida (ex: radiusVelocity)

                // Exemplo: Se for uma nota especial de proximidade, pode ter uma velocity fixa ou maior
                if (noteInfo.type === 'indicator_special') {
                    finalVelocity = 100; // Velocity alta para nota especial
                }

                // Adicionar à fila de notas da forma
                shape.noteQueue.push({
                    note: noteInfo.note,
                    velocity: finalVelocity,
                    channel: shape.midiChannel,
                    entityId: shape.id + 1,
                    type: noteInfo.type, // 'sequential', 'arpeggio', 'chord', 'indicator_special', etc.
                    keyContext: noteInfo.vertexIndex !== undefined ? `${noteInfo.type}_${shape.id}_${noteInfo.vertexIndex}` : `${noteInfo.type}_${shape.id}_${noteInfo.note}_${shape.noteQueue.length}`, // Chave para rastreio
                    playTime: masterBeatCounter + 1 // Tocar na próxima batida do masterBeatCounter (simplificado)
                                                    // Idealmente, playTime seria um timestamp absoluto ou relativo à batida
                });
                logDebug(`Forma ${shape.id}: Nota ${getNoteName(noteInfo.note)} Vel: ${finalVelocity} enfileirada para batida ${masterBeatCounter + 1}. Fila agora com ${shape.noteQueue.length} notas.`);
            });

            // Ativar feedback visual para a forma
            shape.isVisuallyHighlighted = true;
            setTimeout(() => { shape.isVisuallyHighlighted = false; }, VISUAL_HIGHLIGHT_DURATION_MS);


            // Controles MIDI CC (Pitch Bend, Reverb, etc.) ainda são enviados em tempo real, não quantizados.
            // Isso pode ser revisto se a quantização precisar afetar também esses parâmetros.
            if (shape.currentPitchBend !== 8192) sendPitchBend(shape.currentPitchBend, shape.midiChannel);
            const finalReverb = Math.min(127, Math.round(shape.reverbAmount * shape.fxBoostFactor));
            const finalDelay = Math.min(127, Math.round(shape.delayAmount * shape.fxBoostFactor));
            if (finalReverb !== shape.lastSentReverb) { sendMidiCC(91, finalReverb, shape.midiChannel); shape.lastSentReverb = finalReverb; }
            if (finalDelay !== shape.lastSentDelay) { sendMidiCC(94, finalDelay, shape.midiChannel); shape.lastSentDelay = finalDelay; }
            // ... outros CCs ...
        }
    }

    // Processar a fila de notas da forma no tempo certo (masterBeatCounter)
    // Esta parte da lógica será movida para updateMusicalStructureAndTiming ou uma função dedicada
    // para garantir que as notas da fila sejam tocadas no tempo certo.
    // Por agora, a adição à fila está feita. A reprodução da fila será implementada a seguir.

    // A lógica de atualização de lastPitchBend e envio de CCs se mantém, pois são contínuos.
...
function onResults(results) {
  ...
  shapes.forEach(shape => {
    ...
    // v56 - Atualizar tempo de mudança significativa para pausas dinâmicas
    let significantChangeDetected = false;
    if (shape.activeGesture !== null && shape.activeGesture !== shape.lastSentActiveGesture) { // Mudança de gesto
        significantChangeDetected = true;
    }
    if (Math.abs(shape.radius - shape.lastResizeRadius) > 5) { // Mudança de raio
        significantChangeDetected = true;
    }
    // Adicionar verificação de mudança de posição se necessário, mas pode ser muito sensível.
    // Por agora, gesto e raio/lados são os principais.
    // A mudança de lados já é tratada em `shape.lastSideChangeTime`.

    if (significantChangeDetected) {
        shape.lastSignificantChangeTime = performance.now();
    }


    // v56 - Lógica para Gestos Avançados
    shape.fxBoostFactor = 1.0; // Resetar boost a cada frame

    if (shape.activeGesture === 'liquify') {
        shape.lastSignificantChangeTime = performance.now(); // Gesto liquify é atividade
        if (shape.modWheelValue > LIQUIFY_INTENSITY_THRESHOLD_FOR_INVERSION) {
            if (!shape.liquifyIntensityPeaked) {
                shape.nextChordShouldBeInverted = true; // Arma a inversão para o próximo acorde da forma
                shape.liquifyIntensityPeaked = true;
                logDebug(`Forma ${shape.id}: Inversão de acorde armada por liquify intenso.`);
            }
        } else {
            shape.liquifyIntensityPeaked = false;
        }
    }

    if (shape.activeGesture === 'resize' && shape.leftHandLandmarks && shape.rightHandLandmarks) {
        shape.lastSignificantChangeTime = performance.now(); // Gesto resize é atividade
        // Transposição Global pela altura do gesto 'resize'
        const avgHandY = (shape.leftHandLandmarks[0].y + shape.rightHandLandmarks[0].y) / 2; // Normalizado 0-1
        const topThreshold = TRANSPOSE_GESTURE_THRESHOLD_Y_FACTOR;
        const bottomThreshold = 1.0 - TRANSPOSE_GESTURE_THRESHOLD_Y_FACTOR;

        if (avgHandY < topThreshold) {
            if (shape.transpositionGestureState.neutral || shape.transpositionGestureState.lowered) {
                globalKeyTransposeSemits = Math.min(MAX_GLOBAL_TRANSPOSE, globalKeyTransposeSemits + 1);
                logDebug(`Transposição Global: +1 para ${globalKeyTransposeSemits} semitons.`);
                sendOSCMessage('/global/transpose', globalKeyTransposeSemits);
                updateHUD();
            }
            shape.transpositionGestureState = { raised: true, lowered: false, neutral: false };
        } else if (avgHandY > bottomThreshold) {
            if (shape.transpositionGestureState.neutral || shape.transpositionGestureState.raised) {
                globalKeyTransposeSemits = Math.max(-MAX_GLOBAL_TRANSPOSE, globalKeyTransposeSemits - 1);
                logDebug(`Transposição Global: -1 para ${globalKeyTransposeSemits} semitons.`);
                sendOSCMessage('/global/transpose', globalKeyTransposeSemits);
                updateHUD();
            }
            shape.transpositionGestureState = { raised: false, lowered: true, neutral: false };
        } else {
            shape.transpositionGestureState.neutral = true; // Marcar como neutro quando volta para o meio
        }

        // Boost de Efeitos pela velocidade de mudança do raio
        const deltaTime = (performance.now() - (shape.lastResizeTimeForFx || performance.now())) / 1000 || (1/60);
        const radiusChangeSpeed = Math.abs(shape.radius - shape.previousFrameRadius) / deltaTime;
        if (radiusChangeSpeed > FX_BOOST_RADIUS_CHANGE_SPEED_THRESHOLD) {
            shape.fxBoostFactor = FX_BOOST_MULTIPLIER;
            logDebug(`Forma ${shape.id}: FX Boost aplicado (Velocidade Raio: ${radiusChangeSpeed.toFixed(2)})`);
        }
        shape.lastResizeTimeForFx = performance.now();
    }
    shape.previousFrameRadius = shape.radius; // Atualizar para o próximo frame
...
function sendMidiNoteOn(note, velocity, channel, entityId = -1) { // entityId: shape.id+1 ou -2 para cadência
  ...
  if (internalAudioEnabled && currentSimpleSynth && typeof currentSimpleSynth.noteOn === 'function') {
    // Para notas da cadência, podemos querer um tratamento especial no synth (ex: timbre diferente)
    // Por agora, tocamos igual.
    currentSimpleSynth.noteOn(n, v);
  }
  const oscPath = entityId === -2 ? '/cadence/noteOn' : `/forma/${entityId}/noteOn`;
  sendOSCMessage(oscPath, n, v, ch);
  if (dmxSyncModeActive && entityId !== -2) sendOSCMessage(`/dmx/note`, n, v); // DMX Sync só para formas
}

function sendMidiNoteOff(note, channel, entityId = -1) { // entityId: shape.id+1 ou -2 para cadência
  if (spectatorModeActive) return;
  const ch = Math.max(0, Math.min(15, channel));
  const n = Math.max(0, Math.min(127, Math.round(note)));
  ...
  const oscPath = entityId === -2 ? '/cadence/noteOff' : `/forma/${entityId}/noteOff`;
  sendOSCMessage(oscPath, n, ch);
  if (dmxSyncModeActive && entityId !== -2) sendOSCMessage(`/dmx/note`, n, 0); // DMX Sync só para formas
}
...
function turnOffAllActiveNotesForShape(shape, clearActiveNotesObject = true) {
  if (spectatorModeActive) return;
  const origMidiEnabled = midiEnabled; // Salvar estado original
  midiEnabled = true; // Forçar MIDI enabled para garantir que noteOffs sejam enviados se necessário

  logDebug(`Desligando notas ativas para a forma ${shape.id}. Limpar objeto: ${clearActiveNotesObject}`);
  Object.keys(shape.activeMidiNotes).forEach(key => {
    ...
    if (noteInfo && noteInfo.playing) {
      ...
      noteInfo.playing = false; // Marcar como não tocando
    }
    if (noteInfo && noteInfo.staccatoTimer) {
      clearTimeout(noteInfo.staccatoTimer);
      noteInfo.staccatoTimer = null;
    }
    // Se clearActiveNotesObject for true, removeremos a nota do objeto.
    // Caso contrário (ex: ao tocar um novo acorde), mantemos a estrutura para ser sobrescrita.
    if (clearActiveNotesObject) {
      delete shape.activeMidiNotes[key];
    }
  });

  if (clearActiveNotesObject) {
    shape.activeMidiNotes = {}; // Limpa completamente o objeto se solicitado
  }
  midiEnabled = origMidiEnabled; // Restaurar estado original do MIDI
}
...
function turnOffAllActiveNotes() {
  ...
  shapes.forEach(shape => turnOffAllActiveNotesForShape(shape, true)); // true para limpar o objeto de notas ativas

  const currentSimpleSynth = getSimpleSynthInstance();
  if (currentSimpleSynth && typeof currentSimpleSynth.allNotesOff === 'function') {
    currentSimpleSynth.allNotesOff(); // Garante que o synth interno também pare todos os sons
  }
}
...
function setupEventListeners() {
    ...
    // Listeners para Áudio Interno (Modal)
    ...
    // V55: Listener para o novo botão do painel synth (toggleSynthPanelButtonFixed)
    if (toggleSynthPanelButtonFixed && synthControlsSidebar) {
        // Carrega o estado inicial do painel (se está escondido ou não)
        const synthPanelInitiallyHidden = loadPersistentSetting('synthPanelHidden', true); // true = escondido por padrão
        ...
        toggleSynthPanelButtonFixed.addEventListener('click', () => {
            ...
            savePersistentSetting('synthPanelHidden', !isOpen); // Salva o estado 'hidden': se está aberto (!isOpen) é false.
...
// V48: Centralized handler for synth control changes
function handleSynthControlChange(param, value) {
    if (spectatorModeActive) return;
    const synth = getSimpleSynthInstance();
    if (!synth) return;

    switch (param) {
        case 'waveform':
            synth.setWaveform(value);
            if (audioWaveformSelect) audioWaveformSelect.value = value; // Modal
            if (scWaveformSelect) scWaveformSelect.value = value;     // Sidebar
            break;
        case 'masterVolume':
            synth.setMasterVolume(value);
            if (audioMasterVolumeSlider) audioMasterVolumeSlider.value = value; // Modal
            if (audioMasterVolumeValueSpan) audioMasterVolumeValueSpan.textContent = value.toFixed(2); // Modal
            if (scMasterVolumeSlider) scMasterVolumeSlider.value = value;   // Sidebar
            if (scMasterVolumeValue) scMasterVolumeValue.textContent = value.toFixed(2); // Sidebar
            break;
        case 'attack':
            synth.setAttack(value);
            if (audioAttackSlider) audioAttackSlider.value = value; // Modal
            if (audioAttackValueSpan) audioAttackValueSpan.textContent = `${value.toFixed(3)}s`; // Modal
            if (scAttackSlider) scAttackSlider.value = value;       // Sidebar
            if (scAttackValue) scAttackValue.textContent = `${value.toFixed(3)}s`;   // Sidebar
            break;
        case 'decay':
            synth.setDecay(value);
            if (audioDecaySlider) audioDecaySlider.value = value;   // Modal
            if (audioDecayValueSpan) audioDecayValueSpan.textContent = `${value.toFixed(3)}s`; // Modal
            if (scDecaySlider) scDecaySlider.value = value;         // Sidebar
            if (scDecayValue) scDecayValue.textContent = `${value.toFixed(3)}s`;     // Sidebar
            break;
        case 'sustain':
            synth.setSustain(value);
            if (audioSustainSlider) audioSustainSlider.value = value; // Modal
            if (audioSustainValueSpan) audioSustainValueSpan.textContent = value.toFixed(2); // Modal
            if (scSustainSlider) scSustainSlider.value = value;     // Sidebar
            if (scSustainValue) scSustainValue.textContent = value.toFixed(2); // Sidebar
            break;
        case 'release':
            synth.setRelease(value);
            if (audioReleaseSlider) audioReleaseSlider.value = value; // Modal
            if (audioReleaseValueSpan) audioReleaseValueSpan.textContent = `${value.toFixed(3)}s`; // Modal
            if (scReleaseSlider) scReleaseSlider.value = value;     // Sidebar
            if (scReleaseValue) scReleaseValue.textContent = `${value.toFixed(3)}s`;   // Sidebar
            break;
        case 'distortion':
            synth.setDistortion(value);
            if (audioDistortionSlider) audioDistortionSlider.value = value; // Modal
            if (audioDistortionValueSpan) audioDistortionValueSpan.textContent = `${value.toFixed(0)}%`; // Modal
...
// V48: Update Modal Synth Controls from Sidebar or Synth state
function updateModalSynthControls() {
    const synth = getSimpleSynthInstance();
    if (!synth || !settingsModal || settingsModal.style.display !== 'flex') return;

    if (audioWaveformSelect) audioWaveformSelect.value = synth.waveform;
    if (audioMasterVolumeSlider) audioMasterVolumeSlider.value = synth.masterGainNode.gain.value;
    if (audioMasterVolumeValueSpan) audioMasterVolumeValueSpan.textContent = synth.masterGainNode.gain.value.toFixed(2);
    if (audioAttackSlider) audioAttackSlider.value = synth.attackTime;
    if (audioAttackValueSpan) audioAttackValueSpan.textContent = `${synth.attackTime.toFixed(3)}s`;
    if (audioDecaySlider) audioDecaySlider.value = synth.decayTime;
    if (audioDecayValueSpan) audioDecayValueSpan.textContent = `${synth.decayTime.toFixed(3)}s`;
    if (audioSustainSlider) audioSustainSlider.value = synth.sustainLevel;
    if (audioSustainValueSpan) audioSustainValueSpan.textContent = synth.sustainLevel.toFixed(2);
    if (audioReleaseSlider) audioReleaseSlider.value = synth.releaseTime;
...
function updateSidebarSynthControls() {
    const synth = getSimpleSynthInstance();
    if (!synth || !synthControlsSidebar) return;

    if (scWaveformSelect) scWaveformSelect.value = synth.waveform;
    if (scMasterVolumeSlider) scMasterVolumeSlider.value = synth.masterGainNode.gain.value;
    if (scMasterVolumeValue) scMasterVolumeValue.textContent = synth.masterGainNode.gain.value.toFixed(2);
    if (scAttackSlider) scAttackSlider.value = synth.attackTime;
    if (scAttackValue) scAttackValue.textContent = `${synth.attackTime.toFixed(3)}s`;
    if (scDecaySlider) scDecaySlider.value = synth.decayTime;
    if (scDecayValue) scDecayValue.textContent = `${synth.decayTime.toFixed(3)}s`;
    if (scSustainSlider) scSustainSlider.value = synth.sustainLevel;
    if (scSustainValue) scSustainValue.textContent = synth.sustainLevel.toFixed(2);
    if (scReleaseSlider) scReleaseSlider.value = synth.releaseTime;
...
function initSynthControlsSidebar() {
    ...
    // V55: A lógica de inicialização do estado do painel (se está 'open' ou não)
    // e o event listener para toggleSynthPanelButtonFixed já estão em setupEventListeners.
    // A lógica antiga para o botão da sidebar foi removida.

    updateSidebarSynthControls(); // Popula os controles dentro do painel
...
function updateHUD() {
  ...
  const currentAudioCtx = getAudioContext();
  ...
  const currentChordName = currentHarmonicProgression[currentChordInProgressionIndex];
  txt += `Cadência: <span class="highlight">${currentChordName || "N/A"}</span> (Prog: ${HARMONIC_PROGRESSIONS.indexOf(currentHarmonicProgression)+1}/${HARMONIC_PROGRESSIONS.length}) Beat: ${masterBeatCounter % BEATS_PER_CHORD +1}/${BEATS_PER_CHORD}`;
  if (globalKeyTransposeSemits !== 0) {
    txt += ` Transp: <span class="highlight">${globalKeyTransposeSemits > 0 ? '+' : ''}${globalKeyTransposeSemits}st</span>`;
  }
  txt += `<br>`;
...
function toggleOperationMode(){if(spectatorModeActive)return;operationMode=(operationMode==='one_person')?'two_persons':'one_person';/*if(operationModeButton)operationModeButton.textContent=`👤 Modo: ${operationMode==='one_person'?'1P':'2P'}`;*/shapes.forEach(s=>{s.leftHandLandmarks=null;s.rightHandLandmarks=null;s.activeGesture=null;s.lastSentActiveGesture=null;});turnOffAllActiveNotes();updateHUD();saveAllPersistentSettings();} // V55: Removido operationModeButton que não existe mais.
...
function handleKeyPress(e) {
  ...
  const correctedShiftActionMap = {
    ...
    'K': () => { if (oscConfigModal) {oscConfigModal.style.display = oscConfigModal.style.display === 'flex' ? 'none' : 'flex'; if(oscConfigModal.style.display === 'flex') {oscHostInput.value = OSC_HOST; oscPortInput.value = OSC_PORT;}}}, //V55 ensure fields populate
    ...
    'T': toggleTheme,
    'Y': () => { if (toggleSynthPanelButtonFixed) toggleSynthPanelButtonFixed.click(); }, // V55: Atalho para painel do synth (novo botão)
  };
...
function saveAllPersistentSettings(){
  ...
  if(currentSimpleSynth) {
    ...
    if (currentSimpleSynth.delayNode) {
      ...
      savePersistentSetting('delayMix', Math.acos(currentSimpleSynth.delayDryGain.gain.value) / (0.5 * Math.PI));
    }
    if (currentSimpleSynth.reverbDryGain) {
  ...
  savePersistentSetting('synthPanelHidden', synthControlsSidebar ? !synthControlsSidebar.classList.contains('open') : true); // V55: Salva estado baseado na classe 'open'
  console.log("Configs V56 salvas no localStorage.");
}
...
function loadAllPersistentSettings(){
  ...
  spectatorModeActive = false; // Não persistir modo espectador
  ...
  // V56: O estado do painel synth (synthPanelHidden) é carregado e aplicado em setupEventListeners e initSynthControlsSidebar

  console.log("Configs V56 carregadas do localStorage.");
  return {
...
function saveArpeggioSettings(){const s={currentArpeggioStyle,arpeggioBPM,noteInterval,externalBPM};try{localStorage.setItem(ARPEGGIO_SETTINGS_KEY,JSON.stringify(s));}catch(e){}}
...
window.addEventListener('DOMContentLoaded', () => {
    logDebug("DOM Carregado. Iniciando main57.js...");
  console.log("DOM Carregado. Iniciando main57.js (v57)..."); // Log de versão atualizado
    detectPlatform();
    ...
    initPresetManager();
  // setupEventListeners() precisa ser chamado ANTES de initSynthControlsSidebar
  // para que os elementos do DOM (como toggleSynthPanelButtonFixed) estejam disponíveis.
    setupEventListeners();
  initSynthControlsSidebar(); // Agora inicializa os listeners internos dos controles do painel.

    const firstGestureHandler = () => {
        console.log("Primeiro gesto detectado, tentando inicializar o áudio via synth53.js...");
        if (typeof initAudioContextOnGesture === "function") {
          ...
          updateSidebarSynthControls(); // Garante que os valores sejam refletidos
                    console.log("Áudio inicializado/resumido por gesto e synth configurado com valores carregados.");
                } else {
                    console.error("Falha ao obter instância do SimpleSynth após initAudioContextOnGesture.");
                }
        updateHUD(); // Atualiza o HUD, que pode depender do estado do áudio.
        // Remove os listeners de primeiro gesto após sucesso.
                ...
                console.warn("initAudioContextOnGesture() de synth53.js não retornou sucesso. O áudio pode não estar pronto.");
            }
        } else {
            console.error("initAudioContextOnGesture não está definida globalmente (esperada de synth53.js).");
        }
    };
  document.addEventListener('click', firstGestureHandler, { once: false }); // { once: false } para permitir múltiplas tentativas se a primeira falhar por algum motivo
  ...
  // Se o AudioContext já estiver rodando (ex: recarregamento rápido da página ou permissão já concedida),
  // podemos tentar aplicar as configurações de áudio imediatamente.
    const currentSynth = getSimpleSynthInstance();
    if (currentSynth && getAudioContext()?.state === 'running') {
    console.log("AudioContext já estava rodando ou foi ativado rapidamente. Aplicando configurações do synth.");
    ...
    // Removemos os listeners de gesto se o áudio já está OK.
        document.removeEventListener('click', firstGestureHandler);
        document.removeEventListener('keydown', firstGestureHandler);
    console.log("Listeners de primeiro gesto para áudio removidos pois o áudio já está ativo.");
    }
    ...
    if(infoHudButton && hudElement && !hudElement.classList.contains('hidden')) {
  ...
  if (oscLogTextarea) oscLogTextarea.value = `Log OSC - ${new Date().toLocaleTimeString()} - Configs Carregadas (v56).\n`;

  console.log("Iniciando loop de animação (v56) e finalizando DOMContentLoaded.");

    // v56 - Tocar o primeiro acorde da cadência imediatamente
    if (!spectatorModeActive) {
        const firstChordName = currentHarmonicProgression[currentChordInProgressionIndex];
        const firstChordNotes = getChordNotes(firstChordName, CADENCE_BASE_OCTAVE);
        if (firstChordNotes.length > 0) {
            playCadenceChord(firstChordNotes);
        }
        logDebug("Primeiro acorde da cadência tocado na inicialização:", firstChordName);
        lastBeatTime = performance.now(); // Define o tempo base para o próximo beat
    }
...
function animationLoop() {
  updateMusicalStructureAndTiming(); // Processa cadência e alternância de modo
  requestAnimationFrame(animationLoop);
  if (cameraError && !gestureSimulationActive && !camera) {
      drawFallbackAnimation();
      // updateHUD(); // updateHUD já é chamado em onResults ou drawFallbackAnimation
  }
  // Se a câmera não estiver com erro E não estiver em simulação de gestos,
  // onResults -> drawShape -> processShapeNotes já estarão atualizando o HUD.
  // Se houver erro na câmera e não houver simulação, drawFallbackAnimation chama updateHUD.
  // A questão é se o HUD precisa ser atualizado aqui independentemente para a cadência.
  // A cadência já chama updateHUD quando um acorde muda.
  // Chamada explícita ao updateHUD aqui pode ser redundante ou causar flickering.
  // Por ora, vamos confiar que as outras chamadas são suficientes.
}
...
function startAudioRecording() {
    ...
    if (!currentSimpleSynth || !currentSimpleSynth.masterGainNode || !currentAudioCtx) {
    ...
    try {
        ...
        mediaRecorder.onstart = () => {
            ...
            isAudioPaused = false;
            if (recordAudioButton) recordAudioButton.textContent = "⏹️ Parar Gravação";
            if (recordAudioButton) recordAudioButton.classList.add('active');
            if (pauseAudioButton) pauseAudioButton.disabled = false;
            if (pauseAudioButton) pauseAudioButton.textContent = "⏸️ Pausar Gravação";
        ...
        mediaRecorder.onstop = () => {
            if (isAudioRecording) {
                 ...
                 isAudioPaused = false;
                 if (recordAudioButton) recordAudioButton.textContent = "⏺️ Gravar Áudio";
                 if (recordAudioButton) recordAudioButton.classList.remove('active');
                 if (pauseAudioButton) pauseAudioButton.disabled = true;
                 if (pauseAudioButton) pauseAudioButton.textContent = "⏸️ Pausar Gravação";
                 if (saveAudioButton) saveAudioButton.disabled = audioChunks.length === 0;
                 logOSC("SYSTEM", "Gravação de Áudio Parada (onstop)", []);
                 displayGlobalError("Gravação de áudio parada.", 3000);
            }
            try {
              if(currentSimpleSynth && currentSimpleSynth.masterGainNode && destinationNode) {
                currentSimpleSynth.masterGainNode.disconnect(destinationNode);
        ...
        mediaRecorder.onpause = () => {
            if (pauseAudioButton) pauseAudioButton.textContent = "▶️ Retomar Gravação";
            isAudioPaused = true;
        ...
        mediaRecorder.onerror = (event) => {
            ...
            displayGlobalError(`Erro na gravação: ${event.error.name}`, 7000);
            stopAudioRecording();
        };
        ...
        displayGlobalError("Falha ao iniciar gravação: " + e.message, 7000);
        isAudioRecording = false;
    }
}
...
function saveRecordedAudio() {
    ...
    displayGlobalError("Áudio salvo!", 3000);

    audioChunks = [];
    if (saveAudioButton) saveAudioButton.disabled = true;
}

// === END V52: Audio Recording Functions ===
