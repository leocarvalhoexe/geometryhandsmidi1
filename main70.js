...
function sendMidiNoteOn(note, velocity, channel, shapeId = -1, source = 'shapes') {
  ...
  // V70: Controle de fonte de áudio
  if (_internalAudioEnabledMaster && simpleSynth && typeof simpleSynth.noteOn === 'function') {
    if (source === 'shapes' && currentAudioSourceView === 'shapes') {
      simpleSynth.noteOn(n, v);
    } else if (source === 'beatmatrix' && currentAudioSourceView === 'beatmatrix') {
      simpleSynth.noteOn(n, v);
    }
  }
  ...
  if (source === 'shapes') {
    ...
    // OSC específico para beatmatrix pode ser adicionado aqui se necessário
...
function sendMidiNoteOff(note, channel, shapeId = -1, source = 'shapes') {
  ...
  if (source === 'shapes') {
    ...
    // OSC específico para beatmatrix
...
// V70: Função para parar notas da Beat Matrix (se necessário separadamente)
// Por enquanto, o simpleSynth.allNotesOff() global ou o noteOff individual devem ser suficientes
// ao alternar currentAudioSourceView. Se precisarmos de lógica mais granular para
// notas da matrix (por exemplo, se elas não usarem o `shape.activeMidiNotes`),
// uma função dedicada seria implementada aqui.
// function turnOffAllActiveBeatMatrixNotes() { ... }
...
function saveAllPersistentSettings(){
  ...
  savePersistentSetting('currentAudioSourceView', currentAudioSourceView); // V70
  saveBeatMatrixSettings();
  console.log("Configs V70 salvas no localStorage.");
}
...
function loadAllPersistentSettings(){
  ...
  currentAudioSourceView = loadPersistentSetting('currentAudioSourceView', 'shapes'); // V70
...
async function togglePlayPause() {
    ...
    if (isPlaying) {
        // V70: Lógica de controle de áudio
        if (currentAudioSourceView === 'beatmatrix' && isMatrixPlaying) {
            toggleMatrixPlayback(); // Para a beat matrix se estiver tocando
        }
        currentAudioSourceView = 'shapes';
        if (simpleSynth) simpleSynth.allNotesOff(); // Garante que notas da matrix (se houver) parem
        ...
        logDebug("Sequencer de formas iniciado. Fonte de áudio: shapes.");
    } else {
        if (typeof Tone !== 'undefined' && Tone.Transport) { Tone.Transport.pause(); console.log("Tone.Transport paused."); }
        turnOffAllActiveNotes(); // Isso já para as notas das formas
        ...
        logDebug("Sequencer de formas pausado.");
    }
    savePersistentSetting('currentAudioSourceView', currentAudioSourceView); // V70
...
function toggleBeatMatrixVisibility() {
    ...
    if (isBeatMatrixVisible) {
        ...
        if (isPlaying) { // Pausa o sequenciador de formas se estiver tocando
            togglePlayPause(); // Isso já chama turnOffAllActiveNotes()
        } else {
            turnOffAllActiveNotes(); // Garante que notas de formas parem
        }
        ...
        if (isMatrixPlaying) { // Pausa a matrix se estiver tocando
            toggleMatrixPlayback(); // Isso já deve parar as notas da matrix
        } else {
            // Se a matrix não estava tocando, mas poderia ter pads ativos manualmente,
            // precisamos garantir que eles parem se o simpleSynth for global.
            if (simpleSynth) simpleSynth.allNotesOff(); // Para todas as notas do synth
        }
...
function triggerMatrixPad(pad) {
    ...
    if (isActive) {
        if (matrixMidiOut) {
            matrixMidiOut.send([0x90, note, velocity]);
        }
        // V70: Modificado para usar sendMidiNoteOn com a fonte correta
        sendMidiNoteOn(note, velocity, 0, -1, 'beatmatrix'); // Canal 0, shapeId -1 (não aplicável)

        // Para OSC, se desejado:
        // sendOSCMessage(`/beatmatrix/pad/${pad.dataset.note}/on`, velocity);
    } else {
        if (matrixMidiOut) {
            matrixMidiOut.send([0x80, note, 0]);
        }
        // V70: Modificado para usar sendMidiNoteOff com a fonte correta
...
function toggleMatrixPlayback() {
    ...
    // V70: Adiciona controle da fonte de áudio e parada de notas
    if (isMatrixPlaying) {
        currentAudioSourceView = 'beatmatrix';
        turnOffAllActiveNotes(); // Para notas das formas
        logDebug("Matrix playback iniciado. Fonte de áudio: beatmatrix.");
    } else {
        // Se parou a matrix e a visualização principal é de formas, não precisa mudar currentAudioSourceView
        // Mas se a matrix era a fonte ativa, e paramos, idealmente o som para.
        // A lógica em sendMidiNoteOff (chamada pelo stepMatrixSequencer via setTimeout)
        // já deve respeitar currentAudioSourceView.
        // Se o usuário ocultar a matrix enquanto ela toca, toggleBeatMatrixVisibility tomará conta.
        if (simpleSynth && currentAudioSourceView === 'beatmatrix') { // Garante que notas da matrix parem
...
function stepMatrixSequencer() {
    ...
    for (let r = 0; r < currentMatrixNumRows; r++) {
        ...
        if (pad && pad.classList.contains('active')) {
            ...
            // V70: Modificado para usar sendMidiNoteOn e sendMidiNoteOff com a fonte correta
            if (matrixMidiOut) {
                matrixMidiOut.send([0x90, note, velocity]);
            }
