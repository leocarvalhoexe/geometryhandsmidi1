...
let lastSideChangeTime = 0;
const SIDE_CHANGE_DEBOUNCE_MS = 200; // Debounce time in milliseconds
...
let notePlayStyle = 'legato';
let staccatoTimers = {}; // { edgeIndex: timerId }
let pulseAmplitudeFactor = 0.5; // Default 0.0 to 1.0 range
...
let midiEnabled = true; // Toggled by 'M' key or button, initialized to true
...
function turnOffAllActiveNotes() {
    Object.values(staccatoTimers).forEach(clearTimeout);
    staccatoTimers = {};
...
async function initializeCamera() {
  ...
  try {
    // Attempt to get user media to check for permission / device availability
    const stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
    // If successful, release this stream as Camera util will request its own
    stream.getTracks().forEach(track => track.stop());
    console.log("getUserMedia successful, proceeding with MediaPipe Camera - v18 debug");

    // Proceed with original camera setup
    const camera = new Camera(videoElement, {
      onFrame: async () => {
        console.log("Camera onFrame triggered - v18 debug");
        await hands.send({ image: videoElement });
        console.log("hands.send called - v18 debug");
      },
      width: 640,
      height: 480
    });
    console.log("MediaPipe Camera initialized, calling camera.start() - v18 debug");
    camera.start();
...
initializeCamera(); // Call the new async function
...
const drawLandmarks = (landmarks) => {
  console.log("drawLandmarks called with:", landmarks, "- v18 debug");
  const connections = [
    [0,1],[1,2],[2,3],[3,4],
    [0,5],[5,6],[6,7],[7,8],
...
function drawShape(cx, cy, radius, sides, isPulsing, pulseValue) {
  console.log("drawShape called with:", cx, cy, radius, sides, "- v18 debug");
  ctx.beginPath();
  ...
  for (let i = 0; i < sides; i++) {
    ...
    if (midiEnabled && sides > 0) {
        ...
        if (isPulsing) {
            ...
            let pulseIntensity = (pulseValue + 1) / 2; // pulseValue is raw sine, so this is 0 to 1
            let velocityModulationStrength = 0.4 * pulseAmplitudeFactor;
            let dynamicPulseVelocityFactor = (1.0 - velocityModulationStrength) + (pulseIntensity * velocityModulationStrength);
            velocity = Math.round(velocity * dynamicPulseVelocityFactor);
        ...
        if (activeMidiNotes[edgeIndex] && activeMidiNotes[edgeIndex].playing) {
        ...
        } else { // Note is not currently active or not marked as playing
            sendMidiNoteOn(note, velocity, MIDI_CHANNEL);
            activeMidiNotes[edgeIndex] = {
                note: note,
                channel: MIDI_CHANNEL,
                lastVelocity: velocity,
                lastPitchBend: pitchBend, // Store initial pitch bend for staccato
                playing: true
            };
            if (pitchBend !== 8192) {
                sendPitchBend(pitchBend, MIDI_CHANNEL);
            ...
            if (notePlayStyle === 'staccato') {
                // Clear any existing timer for this edge to prevent duplicate noteOffs if retriggered quickly
                if (staccatoTimers[edgeIndex]) {
                    clearTimeout(staccatoTimers[edgeIndex]);
                }
                staccatoTimers[edgeIndex] = setTimeout(() => {
                    if (activeMidiNotes[edgeIndex] && activeMidiNotes[edgeIndex].playing) { // Check if still considered playing
                        sendMidiNoteOff(activeMidiNotes[edgeIndex].note, activeMidiNotes[edgeIndex].channel);
                        activeMidiNotes[edgeIndex].playing = false; // Mark as not playing
                        // Optionally, remove from activeMidiNotes more cleanly here or in the main loop
                    }
                    delete staccatoTimers[edgeIndex]; // Clean up timer reference
                }, 200); // 200ms staccato duration
            }
        }
    }
          ...
          for (const edgeIdxStr of currentActiveEdgeIndices) {
              ...
              if (activeMidiNotes[edgeIdxNum] && activeMidiNotes[edgeIdxNum].playing) {
                  if (edgeIdxNum >= sides) {
                      ...
                      if (staccatoTimers[edgeIdxNum]) {
                        clearTimeout(staccatoTimers[edgeIdxNum]);
                        delete staccatoTimers[edgeIdxNum];
                      }
  ...
  openOutputPopupButton.addEventListener('click', () => {
    if (outputPopupWindow && !outputPopupWindow.closed) {
      ...
      if (!outputPopupWindow || outputPopupWindow.closed || typeof outputPopupWindow.document === 'undefined') {
        alert("Falha ao abrir a janela de saída. O navegador pode ter bloqueado o pop-up ou a janela foi fechada. Por favor, verifique as configurações do seu navegador e permita pop-ups para este site.");
        console.error("Failed to open output popup window. It might be blocked or was closed immediately.");
        outputPopupWindow = null; // Ensure it's null if failed
        popupCanvasCtx = null;
      } else {
        // Proceed with setting up the popup window's content
        ...
        if (popupCanvas) {
          ...
          // Initial size setting might need a slight delay or onload for the new window
          try {
            popupCanvas.width = outputPopupWindow.innerWidth;
            popupCanvas.height = outputPopupWindow.innerHeight;
          } catch (e) {
            console.warn("Error setting initial popup canvas size:", e);
          }
        } else {
          console.error("Could not find 'popupCanvas' in the new window. Closing popup.");
          alert("Erro ao configurar a janela de saída. Tente novamente.");
        ...
        outputPopupWindow.addEventListener('beforeunload', () => {
          ...
          popupCanvasCtx = null;
          outputPopupWindow = null; // Ensure it's nullified here as well
        });
      }
    }
  });
...
function updateHUD() {
  if (hudElement) {
    const radius = Math.round(circleRadius);
    const sides = shapeSides === 100 ? 'Círculo' : shapeSides;
    ...
    hudElement.innerHTML = `
      ...
      Lados: ${sides}<br>
      MIDI: ${midiStatus}<br>
      Pulso: ${pulseStatus}<br>
      Play Style: ${notePlayStyle.charAt(0).toUpperCase() + notePlayStyle.slice(1)}<br>
      ${pulseModeActive ? `Pulso Amp: ${(pulseAmplitudeFactor * 100).toFixed(0)}%` : ""}
    `;
  }
}
...
// Call initially to set button text based on default midiEnabled state
...
if (midiToggleButton) {
  midiToggleButton.addEventListener('click', () => {
    ...
    console.log(`MIDI output ${midiEnabled ? "ENABLED" : "DISABLED"} by button.`);
    updateHUD();
  });
}
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'm' || e.key === 'M') {
    midiEnabled = !midiEnabled;
    updateMidiButtonText(); // Update button text
    if (midiEnabled) {
      console.log("MIDI output ENABLED by key.");
    } else {
      console.log("MIDI output DISABLED by key.");
      turnOffAllActiveNotes();
    }
    updateHUD();
  }
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'l' || e.key === 'L') {
    const oldPlayStyle = notePlayStyle;
    notePlayStyle = (notePlayStyle === 'legato') ? 'staccato' : 'legato';
    console.log(`Note play style switched from ${oldPlayStyle} to: ${notePlayStyle}`);

    if (oldPlayStyle === 'staccato' && notePlayStyle === 'legato') {
      console.log("Switching staccato to legato: Clearing all pending staccato note-off timers.");
      Object.values(staccatoTimers).forEach(clearTimeout);
      staccatoTimers = {};
      // Notes currently in activeMidiNotes that were staccato will now just continue playing
      // as legato notes until their edge is removed or MIDI is toggled.
    }
    updateHUD();
  }
});
