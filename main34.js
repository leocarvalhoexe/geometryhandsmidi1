// ==========================================================================
// MIDI SHAPE MANIPULATOR v34 - main34.js
// ==========================================================================
...
const THEME_STORAGE_KEY = 'midiShapeThemeV34';
const PRESETS_STORAGE_KEY = 'midiShapePresetsV34';
let shapePresets = {};
const APP_SETTINGS_KEY = 'midiShapeManipulatorV34Settings'; // Updated key for v34
const ARPEGGIO_SETTINGS_KEY = 'arpeggioSettingsV34'; // Updated key for v34
...
const resetMidiButton = document.getElementById('resetMidiButton'); // This was missing from original list, ensure it's used or remove
...
async function initializeCamera() {
  try {
    ...
    const camera = new Camera(videoElement, {
      onFrame: async () => {
        if (gestureSimulationActive) {
            if (cameraError && !gestureSimulationActive) { // Should not happen if sim is active
    ...
    displayGlobalError(`Falha webcam/MediaPipe: ${error.message}. Verifique permissões.`, 20000);
    cameraError = true;
    if (!gestureSimulationActive) {
        console.log("Iniciando animação de fallback devido a erro na câmera (simulação de gestos OFF).");
    } else {
        console.log("Erro na câmera, mas simulação de gestos está ATIVA. Fallback visual não será iniciado pela câmera.");
    }
  }
}
...
function setupOSC() {
  ...
  osc.on('close', () => {
    oscStatus = "OSC Desconectado"; console.log(oscStatus);
    if (oscHeartbeatIntervalId) clearInterval(oscHeartbeatIntervalId);
  ...
  osc.on('error', (err) => {
    oscStatus = "OSC Erro"; console.error("OSC Error:", err);
    if (oscHeartbeatIntervalId) clearInterval(oscHeartbeatIntervalId);
    if (reconnectOSCButton) reconnectOSCButton.style.display = 'inline-block';
    updateHUD();
  });
  ...
  try { osc.open(); }
    ...
    console.error("Falha ao iniciar OSC:", error); oscStatus = "OSC Falha ao iniciar";
    updateHUD(); setTimeout(setupOSC, OSC_RECONNECT_TIMEOUT); // Keep auto-retry on initial failure
  }
...
function exportOSCLog() {
    ...
    try {
        ...
        const filename = `osc_log_v34_${timestamp}.txt`;
...
function exportAllPresets() {
    ...
    a.download = `midiShapePresets_v34_${timestamp}.json`;
...
function setupEventListeners() {
    ...
    if (settingsButton) settingsButton.addEventListener('click', () => {if(settingsModal) settingsModal.style.display = 'flex'});
    if (closeSettingsModalButton) closeSettingsModalButton.addEventListener('click', () => {if(settingsModal) settingsModal.style.display = 'none'});
    ...
    if (sendTestOSCButton) sendTestOSCButton.addEventListener('click', () => {
        ...
        sendOSCMessage(address, ...args); logOSC("OUT (Panel)", address, args); oscArgsInput.value = '';
    });
    ...
    if (reconnectOSCButton) reconnectOSCButton.addEventListener('click', () => {
        ...
        if (reconnectOSCButton) reconnectOSCButton.disabled = true;
        setupOSC();
        setTimeout(() => {
            if (osc && osc.status() !== OSC.STATUS.IS_OPEN && reconnectOSCButton) reconnectOSCButton.disabled = false;
...
function updateHUD() {
  ...
  const oscStatusIcon = oscConnected ? '🟢' : '🔴';
  txt += `OSC: ${oscStatusIcon} <span class="${oscConnected ? 'status-ok':'status-error'}">${oscStatus}</span><br>`;
  shapes.forEach(s => {
...
function handleKeyPress(e) {
    ...
    if (e.key === 'Escape') {
        ...
        else if (anyModalOpen) [infoModal, settingsModal, arpeggioSettingsModal, oscControlModal, shapePresetModal].forEach(m => {if(m) m.style.display = 'none'});
        return;
    }
    ...
    const correctedShiftActionMap = { // Renamed from shiftActionMap for clarity
        ...
        'P': playRecordedOSCLoop, // Shift+P for Play Loop OSC
        'S': toggleSpectatorMode, 'T': toggleTheme,
        'B': () => {if(shapePresetModal) shapePresetModal.style.display = shapePresetModal.style.display === 'flex' ? 'none' : 'flex'}, // Shift+B for Presets
    };
...
function saveAllPersistentSettings() {
    ...
    console.log("Configurações persistentes V34 salvas.");
}
function loadAllPersistentSettings() {
    ...
    loadArpeggioSettings();
    console.log("Configurações persistentes V34 carregadas.");
...
window.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Carregado. Iniciando main34.js...");
    hasWebGL2 = checkWebGL2Support(); // Check WebGL support early
    ...
    console.log("main34.js inicialização completa.");
});
// === END INITIALIZATION ===
